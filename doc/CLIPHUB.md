...
ClipHub is the public registry where people share, discover, and download AgentCompany company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to AgentCompany what a package registry is to a programming language. AgentCompany already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in AgentCompany — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own AgentCompany instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the AgentCompany spec. It contains:
...
```
Downloads the template and creates a new company in your local AgentCompany instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within AgentCompany, export your company as a template, then publish:
...
ClipHub is a **separate service** from AgentCompany itself. AgentCompany is self-hosted; ClipHub is a hosted registry that AgentCompany instances talk to.
...
| **AgentCompany CLI** | `agentcompany install`, `agentcompany publish`, `agentcompany cliphub sync` — built into AgentCompany |
| **AgentCompany UI** | "Browse ClipHub" panel in the AgentCompany web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with AgentCompany) |
| Backend | TypeScript + Hono (consistent with AgentCompany) |
| Database | PostgreSQL |
...
4. Run `agentcompany install cliphub:acme/lean-saas-shop`
5. AgentCompany creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in AgentCompany until it works well
2. Export: `agentcompany export --template my-agency`
3. Publish: `agentcompany publish cliphub my-agency`
...
3. Install just that agent: `agentcompany install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to AgentCompany

ClipHub is **not required** to use AgentCompany. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to AgentCompany what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `agentcompany install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in AgentCompany web UI
- [ ] `agentcompany cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with AgentCompany's own philosophy)

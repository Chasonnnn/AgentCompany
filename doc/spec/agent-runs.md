...
This spec defines how AgentCompany actually runs agents while staying runtime-agnostic.
...
1. AgentCompany is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, AgentCompany manages lifecycle, and on exit AgentCompany parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `agentcompany.skill` (shared AgentCompany skill text block)
...
2. Preferred transport is env vars (`AGENTCOMPANY_*`) injected at runtime.

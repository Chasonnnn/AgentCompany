...
For `claude_local` and `codex_local`, AgentCompany assumes the CLI is already installed and authenticated on the host machine.
...
AgentCompany stores resumable session state per `(agent, taskKey, adapterType)`.

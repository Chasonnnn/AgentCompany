# AgentCompany Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for AgentCompany's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main AgentCompany app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary AgentCompany HTTP APIs directly.
...
AgentCompany plugin design is based on the following assumptions:

1. AgentCompany is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core AgentCompany business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by AgentCompany core.
...
2. Let plugins add major capabilities without editing AgentCompany core.
...
4. Depend on project-local plugin folders such as `.agentcompany/plugins`.
...
The single AgentCompany deployment an operator installs and controls.

### 5.2 Company

A first-class AgentCompany business object inside the instance.
...
A trusted in-process extension loaded directly by AgentCompany core.
...
An installable instance-wide extension package loaded through the AgentCompany plugin runtime.
...
AgentCompany has two extension classes.
...
AgentCompany already has a concrete workspace model:
...
Plugins live under the AgentCompany instance directory.

Suggested layout:

- `~/.agentcompany/instances/default/plugins/package.json`
- `~/.agentcompany/instances/default/plugins/node_modules/`
- `~/.agentcompany/instances/default/plugins/.cache/`
- `~/.agentcompany/instances/default/data/plugins/<plugin-id>/`
...
AgentCompany should add CLI commands:

- `pnpm agentcompany plugin list`
- `pnpm agentcompany plugin install <package[@version]>`
- `pnpm agentcompany plugin uninstall <plugin-id>`
- `pnpm agentcompany plugin upgrade <plugin-id> [version]`
- `pnpm agentcompany plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@agentcompany/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@agentcompany/plugin-linear",
  "version": "0.1.0",
  "agentcompanyPlugin": {
...
```ts
export interface AgentCompanyPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that AgentCompany agents can use during runs.
...
- AgentCompany server starts one worker process per installed plugin
...
Receives one typed AgentCompany domain event.
...
export function definePlugin(definition: PluginDefinition): AgentCompanyPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: AgentCompanyPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `/plugin-sdk/ui`
...
The `/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the AgentCompany secret provider system rather than a plain text input
...
1. Core AgentCompany data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing AgentCompany objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual AgentCompany product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if AgentCompany later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing AgentCompany secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm agentcompany plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the AgentCompany server. This is a normative requirement, not optional.
...
```ts
import { createTestHarness } from "/plugin-test-harness";
...
For developing a plugin against a running AgentCompany instance:

- The operator installs the plugin from a local path: `pnpm agentcompany plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-agentcompany-plugin`) that scaffolds:

- `package.json` with correct `agentcompanyPlugin` keys
...
- `@agentcompany/plugin-workspace-files`
- `@agentcompany/plugin-terminal`
- `@agentcompany/plugin-git`
- `@agentcompany/plugin-linear`
- `@agentcompany/plugin-github-issues`
- `@agentcompany/plugin-grafana`
- `@agentcompany/plugin-runtime-processes`
- `@agentcompany/plugin-stripe`
...
- `/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `/plugin-sdk/ui`
...
- `/plugin-test-harness`
- `create-agentcompany-plugin` starter template
...
AgentCompany should not implement a generic in-process hook bag modeled directly after local coding tools.

AgentCompany should implement:
...
That is the complete target design for the AgentCompany plugin system.

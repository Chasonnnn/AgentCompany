...
This guide describes the current, implemented way to create a AgentCompany plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main AgentCompany app.
...
```bash
pnpm --filter /create-agentcompany-plugin build
node packages/plugins/create-agentcompany-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the AgentCompany repo:

```bash
pnpm --filter /create-agentcompany-plugin build
node packages/plugins/create-agentcompany-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `/plugin-sdk` from the local AgentCompany checkout into a `.agentcompany-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into AgentCompany from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `/plugin-sdk/ui`

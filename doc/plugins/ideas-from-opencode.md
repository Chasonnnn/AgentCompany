...
AgentCompany V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a AgentCompany-shaped design.

Assumption for this document: AgentCompany is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class AgentCompany objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into AgentCompany.

The main conclusion is:

- AgentCompany should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- AgentCompany should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- AgentCompany should use multiple extension classes instead of one generic plugin bag:
...
If AgentCompany does this well, the examples you listed become straightforward:
...
Relevant AgentCompany files reviewed for current extension seams:
...
It is too dangerous for AgentCompany core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for AgentCompany — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
AgentCompany should absolutely do this.
...
AgentCompany should copy this discipline.
...
AgentCompany should adopt this style for plugin actions, automations, and UI schemas.
...
AgentCompany can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for AgentCompany too.

## What AgentCompany Should Not Copy Directly
...
AgentCompany is a control plane for an operator-managed instance with company objects.
...
AgentCompany is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

AgentCompany should not auto-load arbitrary code from a workspace repo like `.agentcompany/plugins` or project directories.
...
For AgentCompany, equivalent hooks into:
...
AgentCompany should not allow plugins to silently replace:
...
For AgentCompany it would be risky because it combines:
...
AgentCompany should require an explicit operator install step.

## Why AgentCompany Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | AgentCompany |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted AgentCompany instance |
...
That means AgentCompany should borrow the good ideas from `opencode` but use a stricter architecture.

## AgentCompany Already Has Useful Pre-Plugin Seams

AgentCompany has several extension-like seams already:
...
This is good news.
AgentCompany does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended AgentCompany Plugin Model
...
AgentCompany already has this pattern implicitly:
...
- `plugin` means integration code that talks to AgentCompany through a typed plugin protocol and capability model
...
- invoke explicit AgentCompany actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`/plugin-sdk/ui`) with design tokens and common components.
...
AgentCompany should treat plugin installation as a global instance-level action.

Examples:

- install `@agentcompany/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over AgentCompany objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

AgentCompany already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for AgentCompany too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@agentcompany/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running AgentCompany instance.

A `create-agentcompany-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how AgentCompany achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the AgentCompany server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. AgentCompany's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For AgentCompany

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "/plugin-sdk";

export default definePlugin({
  id: "@agentcompany/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit AgentCompany entities
    });
    ...
    ctx.events.on("plugin.@agentcompany/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "/plugin-sdk/ui";
...
Any AgentCompany plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific AgentCompany objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@agentcompany/plugin-linear`)
...
- reuse core tables when the data is clearly part of AgentCompany itself
...
If a concept is becoming part of AgentCompany's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of AgentCompany itself
...
### 3. Scope plugin data to AgentCompany objects before adding custom schemas

A lot of plugin data naturally hangs off existing AgentCompany objects:
...
If AgentCompany eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@agentcompany/plugin-workspace-files`
...
Package idea: `@agentcompany/plugin-terminal`
...
Package idea: `@agentcompany/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving AgentCompany.
- Board reviews the diff after a run without leaving AgentCompany.
...
- optional `events.emit` (e.g. `plugin.@agentcompany/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@agentcompany/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@agentcompany/plugin-linear`

This plugin syncs AgentCompany work with Linear. It is useful for:

- importing backlog from Linear
- linking AgentCompany issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between AgentCompany and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into AgentCompany.
- AgentCompany issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@agentcompany/plugin-github-issues`

This plugin syncs AgentCompany issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to AgentCompany issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with AgentCompany task governance
...
- Plugin settings:
  ...
  - whether new AgentCompany issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active AgentCompany issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into AgentCompany.
- GitHub webhooks update status/comment state in AgentCompany.
- A PR is linked back to the AgentCompany issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@agentcompany/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@agentcompany/plugin-grafana`

This plugin surfaces external metrics and dashboards inside AgentCompany. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating AgentCompany issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full AgentCompany plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the AgentCompany dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a AgentCompany issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of AgentCompany core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@agentcompany/plugin-runtime-processes`
...
Package idea: `@agentcompany/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into AgentCompany. It is useful for:
...
- Failed payment spikes or churn events can generate AgentCompany issues for follow-up.
...
- Stripe data should stay additive to AgentCompany core
...
- plugin UI bundle loading, host bridge, `/plugin-sdk/ui`
...
- `/plugin-test-harness` and `create-agentcompany-plugin` starter template
...
AgentCompany should not implement "an OpenCode-style generic in-process hook system."
AgentCompany should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In AgentCompany
...
8. Ship `/plugin-test-harness` and `create-agentcompany-plugin` starter template.

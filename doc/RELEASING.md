# Releasing AgentCompany

Maintainer runbook for shipping AgentCompany across npm, GitHub, and the website-facing changelog surface.
...
AgentCompany uses calendar versions that still fit semver syntax:
...
2. **npm** — `agentcompany` and public workspace packages are published
...
# or
npx agentcompany@canary onboard --data-dir "$(mktemp -d /tmp/paperclip-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/chason/agentcompany/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for AgentCompany. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
- `npx agentcompany@canary onboard` installs

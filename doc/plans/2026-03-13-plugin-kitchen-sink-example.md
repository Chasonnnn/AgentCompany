...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented AgentCompany plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside AgentCompany.
...
- package: `/plugin-kitchen-sink-example`
- plugin id: `agentcompany.kitchen-sink-example` or `agentcompany-kitchen-sink-example`

Recommendation: use `agentcompany-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core AgentCompany event such as `issue.created`
...
Recommendation: default to a AgentCompany-local endpoint or a stable public echo endpoint to avoid flaky docs.

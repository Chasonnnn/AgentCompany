...
Agents need API keys to authenticate with AgentCompany. The current approach
...
**Trust model:** The adapter process runs on the same machine as the AgentCompany
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** AgentCompany generates a token and passes it directly to the agent
...
`AGENTCOMPANY_API_URL`, `AGENTCOMPANY_AGENT_ID`, `AGENTCOMPANY_COMPANY_ID`. We need to
add a `AGENTCOMPANY_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a AgentCompany CLI
...
# Opens browser -> user confirms -> token stored at ~/.agentcompany/credentials
```
...
managed by the AgentCompany server itself.
...
1. A company admin (user or agent) generates an **invite URL** from AgentCompany.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The AgentCompany SKILL.md (or a link to it)
   - What information AgentCompany needs from the agent (e.g. webhook URL, adapter
...
5. AgentCompany stores the pending registration.
...
7. On approval, AgentCompany provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** AgentCompany issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.agentcompany.ing/skills/agentcompany/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How AgentCompany should send you heartbeats",
...
1. Generate a AgentCompany invite link for the company.
...
4. A AgentCompany company member approves the new agent.
5. AgentCompany begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `AGENTCOMPANY_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `agentcompany auth login` for developer-managed remote agents.                                      |

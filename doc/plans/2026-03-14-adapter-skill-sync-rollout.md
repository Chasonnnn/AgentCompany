...
This document defines the rollout plan for adapter-wide skill support in AgentCompany.
...
- AgentCompany stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

AgentCompany currently has these adapters:
...
1. Desired skills live in AgentCompany for every adapter.
...
These adapters have a stable local skills directory that AgentCompany can read and manage.
...
These adapters do not have a meaningful AgentCompany-owned persistent install state.
...
- show desired AgentCompany skills
...
- desired skills stored in AgentCompany
...
- runtime already injects AgentCompany skills into `~/.cursor/skills`
...
- runtime already injects AgentCompany skills into `~/.gemini/skills`
...
- runtime already injects AgentCompany skills into `~/.pi/agent/skills`
...
- AgentCompany can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects AgentCompany skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- AgentCompany may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed AgentCompany skills that are clearly marked as AgentCompany-managed
...
- AgentCompany stores desired skills only
...
- avoid destructive wording unless AgentCompany can prove a skill is AgentCompany-managed
...
That gets AgentCompany from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.

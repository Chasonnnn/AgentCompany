...
AgentCompany needs to support two very different but equally valid ways of working:
...
Today, AgentCompany already has the beginnings of this model:
...
- making AgentCompany feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing AgentCompany users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted AgentCompany deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning AgentCompany into a full code review product
...
This object must be recorded explicitly so that AgentCompany can:
...
AgentCompany should treat PRs as a type of work product linked back to:
...
AgentCompany already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted AgentCompany must be a first-class deployment mode

AgentCompany cannot assume that it is running on the same machine as the code.

In cloud deployments, AgentCompany may:
...
- `Runtime service`: a process or service AgentCompany owns or tracks for a workspace
...
It also lets cloud-hosted AgentCompany deployments point at codebases and remotes without pretending the AgentCompany host has direct filesystem access.
...
This lets AgentCompany support:
...
Without an explicit `execution workspace` record, AgentCompany has nowhere stable to attach:
...
AgentCompany needs a single place to show:
...
- AgentCompany-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without AgentCompany owning the execution host
...
## 1. AgentCompany host and execution host must be decoupled

AgentCompany may run:
...
without the AgentCompany host itself running local git or local preview processes.
...
AgentCompany should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for AgentCompany.
...
AgentCompany should keep workspace automation centered on a small set of hooks:
...
AgentCompany should provide the equivalent visibility in the project `Code` surface:
...
AgentCompany should treat execution workspaces as the natural home for:
...
AgentCompany should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
AgentCompany should continue treating cleanup policy as part of the core workspace model:
...
AgentCompany should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the AgentCompany host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

AgentCompany should borrow Worktrunk's good execution patterns while keeping the broader AgentCompany model:
...
This is how AgentCompany supports:
...
That keeps AgentCompany focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted AgentCompany deployments remain first-class

That is a better fit for AgentCompany than either extreme:

...
Date: 2026-03-13  
Related discussion: https://github.com/chason/agentcompany/discussions/449
...
- local runtime data from the default AgentCompany instance on 2026-03-13
...
- AgentCompany discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing AgentCompany skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after AgentCompany-side skill changes land.
...
- `skills/agentcompany/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/agentcompany-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - AgentCompany-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching AgentCompany to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
- For `codex_local`, either:
  ...
  - treat AgentCompany-owned Codex skill symlinks as repairable when they point at a different checkout
...
6. Rewrite `skills/agentcompany/SKILL.md` around delta-fetch behavior.

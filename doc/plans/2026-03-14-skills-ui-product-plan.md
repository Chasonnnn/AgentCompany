...
This document defines the product and UI plan for skill management in AgentCompany.
...
- AgentCompany company import/export can include skills as package content
...
3. The UI must always tell the truth about what AgentCompany knows:
   - desired state in AgentCompany
...
AgentCompany should treat skills at two scopes:
...
- created locally inside AgentCompany later
...
7. “Understand what AgentCompany knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a AgentCompany-specific package layout
...
This matters because Codex and similar adapters may already have local skills that AgentCompany did not install.

These should be clearly marked:

- external
- not managed by AgentCompany
...
- `desired_only`: AgentCompany wants it, adapter does not show it yet
- `external`: adapter has it but AgentCompany does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, AgentCompany only tracks desired state
...
- AgentCompany can still track desired skills
...
- AgentCompany can see adapter skills
...
- `.agentcompany.yaml` may add AgentCompany-specific fidelity, but should not replace the base shortname association model
...
- `.agentcompany.yaml` may add AgentCompany-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
AgentCompany should have a company-scoped skill model or managed package model representing:
...
AgentCompany should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too AgentCompany-specific.
...
That gives AgentCompany one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.

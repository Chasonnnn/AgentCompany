...
AgentCompany's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends AgentCompany's API, UI, and data model | Yes |
...
modules/
  observability/
    agentcompany.module.json     # manifest (required)
    src/
...
### Manifest (`agentcompany.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`/mod-observability`)
...
```typescript
import type { ModuleAPI } from "/core";
...
```
1. Scan modules/ directory for agentcompany.module.json manifests
...
```jsonc
// agentcompany.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "templates": [
    {
      ...
      "url": "https://store.agentcompany.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm agentcompany store list                    # browse available modules and templates
pnpm agentcompany store install <module-id>     # install a module
pnpm agentcompany store import <template-id>    # import a company template
pnpm agentcompany store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one AgentCompany instance | `server:started` |
...
4. **Module config** — `agentcompany.config.json` with per-module config, env var interpolation
...
10. **`/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema

...
Add a single storage subsystem for AgentCompany that supports:
...
- API serves attachment bytes through authenticated AgentCompany endpoints.
...
- Default local path is under instance root: `~/.agentcompany/instances/<instanceId>/data/storage`.
...
- `agentcompany onboard` writes a valid `storage` config block by default.
- `agentcompany configure --section storage` can switch between local and s3 modes.
...
- `agentcompany doctor` reports actionable storage status.

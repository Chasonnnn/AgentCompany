...
- New config/env settings:
  - `AGENTCOMPANY_AGENT_JWT_SECRET`
  - `AGENTCOMPANY_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `AGENTCOMPANY_AGENT_JWT_ISSUER` (default: `paperclip`)
  - `AGENTCOMPANY_AGENT_JWT_AUDIENCE` (default: `agentcompany-api`)
...
1. In:
   ...
   inject `AGENTCOMPANY_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `AGENTCOMPANY_API_KEY`, do not overwrite it.
- Continue injecting:
...
- Local adapters authenticate without manual `AGENTCOMPANY_API_KEY` config.

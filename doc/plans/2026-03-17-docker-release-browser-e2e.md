...
Today release smoke testing for published AgentCompany packages is manual and shell-driven:
...
- Docker
- `npx agentcompany@canary`
- `npx agentcompany@latest`
...
- runs `agentcompany@${PAPERCLIPAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@agentcompany.local`
...
1. A published `agentcompany@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `agentcompany@latest` Docker install can be smoke-tested by Playwright in CI.

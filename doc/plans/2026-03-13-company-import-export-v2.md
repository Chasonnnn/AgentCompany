...
This document defines the next-stage plan for AgentCompany company import/export.

The core shift is:

- move from a AgentCompany-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside AgentCompany.
...
AgentCompany already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just AgentCompany
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `agentcompany.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.agentcompany.yaml` sidecar for high-fidelity AgentCompany-specific details
...
11. adapter-aware skill sync surfaces so AgentCompany can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point AgentCompany at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a AgentCompany sidecar.
...
AgentCompany must not redefine `SKILL.md`.
...
- AgentCompany-specific fidelity lives in `.agentcompany.yaml`
- AgentCompany may resolve and install `SKILL.md` packages, but it must not require a AgentCompany-only skill format
...
### 5.4 Base Package Vs AgentCompany Extension
...
- base package:
  ...
  - no AgentCompany-only runtime fields by default
- AgentCompany extension:
  - `.agentcompany.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by AgentCompany tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`agentcompany.manifest.json` is not part of the future package direction.
...
AgentCompany import/export should support these entity kinds:
...
In AgentCompany V2 portability:
...
- AgentCompany can treat “this set of agents came from team package X” as the imported-team model
...
AgentCompany should support source references in package metadata with:
...
AgentCompany should define a new adapter surface area around skills:
...
Planned AgentCompany behavior:

- if an adapter supports read, AgentCompany should show current skills in the UI
- if an adapter supports write, AgentCompany should let the user enable/disable imported skills
- if an adapter supports sync, AgentCompany should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.agentcompany.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.agentcompany.yaml` alongside the base package
...
## 10. Storage Model Inside AgentCompany
...
AgentCompany should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `agentcompany company export <company-id> --out <path>`
- `agentcompany company import <path-or-url> --dry-run`
- `agentcompany company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in AgentCompany storage, or only referenced at import time?
...
3. Should AgentCompany support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps AgentCompany aligned with:

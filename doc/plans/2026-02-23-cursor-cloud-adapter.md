...
This document defines the V1 design for a AgentCompany adapter that integrates with
...
4. synthesized stdout events for AgentCompany UI/CLI
...
3. **Agent callback auth to AgentCompany** uses a bootstrap exchange flow (no long-lived AgentCompany key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from AgentCompany endpoints, not full SKILL.md prompt injection.
...
- `agentcompanyPublicUrl` (optional override; else `AGENTCOMPANY_PUBLIC_URL` env)
...
## AgentCompany Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `AGENTCOMPANY_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.agentcompanyPublicUrl`
2. `process.env.AGENTCOMPANY_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived AgentCompany credentials in prompt text.

Flow:

1. Before launch/follow-up, AgentCompany mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `agentcompanyPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. AgentCompany validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all AgentCompany API calls.
...
1. Prompt includes a compact instruction to fetch skills from AgentCompany.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/agentcompany`
   - `GET /api/skills/agentcompany-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <agentcompanyPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active AgentCompany run context.
...
5. `agentcompanyPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from AgentCompany skill endpoints |
| AgentCompany auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from AgentCompany context.
3. Add richer PR metadata surfacing in AgentCompany UI.

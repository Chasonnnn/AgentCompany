...
`PAP-447` asks how AgentCompany should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same AgentCompany instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into AgentCompany:

- some operators use AgentCompany to manage AgentCompany and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as AgentCompany
- Claude and Codex expose different built-in affordances, so AgentCompany should not overfit to one tool

## Core Product Decision

AgentCompany should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by AgentCompany
...
- AgentCompany should not assume that every adapter can see or use a host filesystem path directly
...
If AgentCompany makes worktrees agent-first, it will blur:
...
- AgentCompany core resolves and records execution workspace state
...
That gives AgentCompany room to support:
...
- AgentCompany handles workspace runtime services under the hood using project/workspace policy
...
Once AgentCompany is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So AgentCompany should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that AgentCompany cannot infer
...
- when issue moves to done, should AgentCompany auto-commit?
- should it auto-push?
...
AgentCompany already has the right foundation for a project-first model.
...
For server-spawned local adapters, AgentCompany already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that AgentCompany should mirror directly

Implication:

- for `codex_local`, AgentCompany should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but AgentCompany should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, AgentCompany should separate:
...
AgentCompany should be able to express intentions such as:
...
These are not specific to AgentCompany. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".agentcompany/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a AgentCompany-specific `devServer` flag.
...
AgentCompany should distinguish between:
...
AgentCompany should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, AgentCompany should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to AgentCompany
...
For runtime services specifically, AgentCompany should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as AgentCompany-only dev-server flags
...
3. Inject resolved execution workspace details into `context.agentcompanyWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / AgentCompany instance can be reused through env/config injection when appropriate
...
- AgentCompany can decide whether to reuse or start a fresh service deterministically
...
3. Test against AgentCompany-on-AgentCompany workflows first.
...
7. Workspace runtime services are modeled generically, not as AgentCompany-specific dev-server toggles.

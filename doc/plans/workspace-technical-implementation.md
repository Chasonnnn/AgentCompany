...
This repo already uses `AGENTCOMPANY_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `AGENTCOMPANY_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - AgentCompany may create host-local worktrees, processes, and paths
- `cloud`
  - AgentCompany should assume no durable host-local execution workspace management
...
- use `AGENTCOMPANY_EXECUTION_TOPOLOGY` as a defaulting guardrail

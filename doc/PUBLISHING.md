...
Low-level reference for how AgentCompany packages are prepared and published to npm.
...
AgentCompany no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `agentcompany`, imports code from workspace packages such as:
...
- If `npm view /ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `/ui`, the `agentcompany` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
AgentCompany uses calendar versions:
...
- `agentcompany@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx agentcompany@canary onboard
```

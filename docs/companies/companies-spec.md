...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only AgentCompany.
...
- be useful outside AgentCompany
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.agentcompany.yaml`
...
Example AgentCompany extension:
...
- AgentCompany must not require extra top-level fields for skill validity
- AgentCompany-specific extensions must live under `metadata.agentcompany` or `metadata.sources`
...
For AgentCompany, the preferred fidelity extension is:
...
- issue/task AgentCompany-only metadata
...
- AgentCompany tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested AgentCompany shape:
...
Additional rules for AgentCompany exporters:
...
- omit empty and default-valued AgentCompany fields when possible
...
- AgentCompany exporters should emit `.agentcompany.yaml` as a sidecar by default
...
## 19. AgentCompany Mapping

AgentCompany can map this spec to its runtime model like this:
...
- AgentCompany extension:
  - `.agentcompany.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other AgentCompany-specific fidelity

Inline AgentCompany-only metadata that must live inside a shared markdown file should use:

- `metadata.agentcompany`

That keeps the base format broader than AgentCompany.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only AgentCompany.

## 20. Cutover

AgentCompany should cut over to this markdown-first package model as the primary portability format.

`agentcompany.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For AgentCompany, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.

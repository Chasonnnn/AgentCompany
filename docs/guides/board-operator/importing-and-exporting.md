...
AgentCompany companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .agentcompany.yaml     # Adapter config, env inputs, routines
```
...
- **.agentcompany.yaml** holds AgentCompany-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
agentcompany company export <company-id> --out ./my-export
```
...
agentcompany company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
agentcompany company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
agentcompany company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.agentcompany.yaml`
...
```sh
# From a local folder
agentcompany company import ./my-export

# From a GitHub URL
agentcompany company import https://github.com/org/repo

# From a GitHub subfolder
agentcompany company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
agentcompany company import org/repo
agentcompany company import org/repo/companies/acme
```
...
If `--target` is not specified, AgentCompany infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
agentcompany company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
agentcompany company import org/company-templates/engineering-team \
  --target new \
...
```sh
agentcompany company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
agentcompany company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
agentcompany company import ./package \
  --target new \
  --yes \
  --json
```
...
AgentCompany supports several GitHub URL formats:

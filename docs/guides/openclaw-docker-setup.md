...
How to get OpenClaw running in a Docker container for local development and testing the AgentCompany OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

AgentCompany includes an end-to-end join smoke harness:
...
- probes and prints a AgentCompany host URL that is reachable from inside OpenClaw Docker
...
- `AGENTCOMPANY_HOST_PORT` (default `3100`)
- `AGENTCOMPANY_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your AgentCompany deployment is `authenticated`, provide auth context:

```bash
AGENTCOMPANY_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
AGENTCOMPANY_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host AgentCompany server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed AgentCompany URL (typically `http://host.docker.internal:3100`).
- If AgentCompany rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm agentcompany allowed-hostname host.docker.internal
```

Then restart AgentCompany and rerun the smoke script.

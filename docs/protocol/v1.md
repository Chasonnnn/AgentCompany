...
`usage.reconciliation.record` params:

- `workspace_dir` (required)
- `statement_id` (optional; upsert key)
- `provider` (required)
- `period_start` (required ISO datetime)
- `period_end` (required ISO datetime)
- `billed_cost_usd` (required, non-negative)
- `billed_tokens` (optional, non-negative integer)
- `currency` (optional; currently `USD` only)
- `source` (optional; `manual|api`)
- `external_ref` (optional)
- `notes` (optional)

`usage.reconciliation.snapshot` params:
...
- `workers_per_dept` (optional integer; default `2`, min `1`)
...
`workspace.enterprise.ensure_defaults` params:
...
Result semantics:
- `bootstrapped=true` when empty workspace was auto-seeded with enterprise topology.
- `ready=false` with `reason=skipped_existing_non_enterprise_topology` when existing non-empty topology is detected and no destructive rewrite is performed.
...
Enterprise startup semantics:
- When called in workspace scope on an empty workspace (no projects/teams and no Executive Manager), the server auto-runs enterprise bootstrap once before assembling the snapshot.
- Auto-bootstrap seeds default enterprise topology (Executive Manager + department director/worker teams + initial project) so staffing exists before project workflows.
...
- `view_data`:
  - `home`: `{ workspace_home, pm, resources, recommendations[], forecast? }`
  - `activities`: `{ ui }`
  - `resources`: `{ resources }`
...
- `reconciliation` includes statement-vs-runtime cost/token deltas (`totals`, `by_provider[]`, coverage counters)
...
`pm.recommend_allocations` response highlights:

- `recommendations[]` per task (`preferred_provider`, `preferred_model`, `preferred_agent_id`, `token_budget_hint`, `rationale`)
- `forecast.mode` = `simulation_v1`
- `forecast.baseline` and `forecast.recommended` provide projected span/tokens/cost and capacity pressure
- `forecast.scenarios[]` includes `throughput_bias` and `cost_bias` alternatives for decision support

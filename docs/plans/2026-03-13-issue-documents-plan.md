...
Add first-class **documents** to AgentCompany as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by AgentCompany workflow/docs.
...
Update the AgentCompany agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/agentcompany/SKILL.md`

...
Build a custom adapter to connect AgentCompany to any agent runtime.
...
|---|---|---|
| Source | Inside `agentcompany-fork` | Separate npm package |
| Distribution | Ships with AgentCompany | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying AgentCompany's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `/adapter-utils/server-utils`
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildPaperclipEnv(agent)` | `/adapter-utils/server-utils` | Inject `AGENTCOMPANY_*` env vars |
| `renderTemplate(tpl, data)` | `/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in AgentCompany's source:
...
`format-event.ts` — pretty-prints stdout for `agentcompany run --watch` using `picocolors`.
...
Make AgentCompany skills discoverable to your agent runtime without writing to the agent's working directory:

...
summary: Build, package, and distribute adapters as plugins without modifying AgentCompany source
---

AgentCompany supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to AgentCompany's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `agentcompany-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with AgentCompany | Published to npm or linked via `file:` |
| Updates | Requires AgentCompany release | Independent versioning |
...
```ts
import type { ServerAdapterModule } from "/adapter-utils";
...
import {
  ...
  buildPaperclipEnv,
  renderTemplate,
} from "/adapter-utils/server-utils";
...
export async function execute(
  ...
  // 2. Build environment with AgentCompany vars injected
...
#### Available Helpers from `/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildPaperclipEnv(agent)` | Inject `AGENTCOMPANY_*` environment variables |
...
```sh
# Via the AgentCompany UI
# Settings → Adapters → Install from npm → "my-agentcompany-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-agentcompany-adapter"}'
```
...
Local adapters are symlinked into AgentCompany's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.agentcompany/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-agentcompany-adapter",
...
```ts
import type { AdapterSessionCodec } from "/adapter-utils";
...
Other AgentCompany users can then install your adapter by package name from the UI or API.

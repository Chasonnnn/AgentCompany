...
AgentCompany exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`AGENTCOMPANY_API_KEY`)
...
- Run audit trail: include `X-AgentCompany-Run-Id` header on all mutating requests during heartbeats

...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-AgentCompany-Signature` + `X-AgentCompany-Timestamp` header pair matching the trigger's signing mode.

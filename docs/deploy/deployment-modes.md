...
AgentCompany supports two runtime modes with different security profiles.
...
```sh
pnpm agentcompany allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, AgentCompany emits a one-time claim URL at startup:
...
```sh
AGENTCOMPANY_DEPLOYMENT_MODE=authenticated pnpm agentcompany run
```

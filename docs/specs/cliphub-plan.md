# ClipHub: Marketplace for AgentCompany Team Configurations
...
> The "app store" for whole-company AI teams — pre-built AgentCompany configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for AgentCompany-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | AgentCompany company (agents, projects, governance) |
...
A complete AgentCompany company configuration:
...
Single-agent configurations designed to plug into a AgentCompany org:
...
Portable skill files that any AgentCompany agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with AgentCompany's skill loading system
...
interface Listing {
  ...
  agentcompanyVersionMin: string;     // Minimum AgentCompany version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target AgentCompany company
...
| `POST` | `/api/listings/:id/install` | Install to AgentCompany company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target AgentCompany company (or create new)
  → ClipHub API calls AgentCompany API to:
  ...
  → Redirect to AgentCompany dashboard with new team running
```
...
### 5.3 Creator: Export from AgentCompany → Publish

```
Running AgentCompany company → "Export as Blueprint" (CLI or UI)
  → AgentCompany exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (AgentCompany brand blue/purple)
...
| Install flow | One-click deploy to AgentCompany company |
...
{
  "targetCompanyId": "uuid",         // Existing AgentCompany company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `agentcompany-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as AgentCompany
- **Backend**: Node.js API (or extend AgentCompany server)
- **Database**: Postgres (can share AgentCompany's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with AgentCompany auth (or OAuth2)

### 9.2 Integration with AgentCompany

ClipHub can be:
- **Option A**: A separate app that calls AgentCompany's API to install blueprints
- **Option B**: A built-in section of the AgentCompany UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing AgentCompany UI and API.
...
- [ ] Install flow: blueprint → AgentCompany company
...
- [ ] "Export from AgentCompany" CLI command

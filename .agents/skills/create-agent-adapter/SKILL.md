...
description: >
  Technical guide for creating a new AgentCompany agent adapter. Use when building
...
# Creating a AgentCompany Agent Adapter

An adapter bridges AgentCompany's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `agentcompany run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `/adapter-utils` (types) or `/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell AgentCompany to forget the stored session
}
...
The `agentConfigurationDoc` is read by LLM agents (including AgentCompany agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `/adapter-utils/server-utils`)
2. **Build environment** — call `buildPaperclipEnv(agent)` then layer in `AGENTCOMPANY_RUN_ID`, context vars (`AGENTCOMPANY_TASK_ID`, `AGENTCOMPANY_WAKE_REASON`, `AGENTCOMPANY_WAKE_COMMENT_ID`, `AGENTCOMPANY_APPROVAL_ID`, `AGENTCOMPANY_APPROVAL_STATUS`, `AGENTCOMPANY_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `AGENTCOMPANY_AGENT_ID` | `agent.id` |
| `AGENTCOMPANY_COMPANY_ID` | `agent.companyId` |
| `AGENTCOMPANY_API_URL` | Server's own URL |
| `AGENTCOMPANY_RUN_ID` | Current run id |
| `AGENTCOMPANY_TASK_ID` | `context.taskId` or `context.issueId` |
| `AGENTCOMPANY_WAKE_REASON` | `context.wakeReason` |
| `AGENTCOMPANY_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `AGENTCOMPANY_APPROVAL_ID` | `context.approvalId` |
| `AGENTCOMPANY_APPROVAL_STATUS` | `context.approvalStatus` |
| `AGENTCOMPANY_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `AGENTCOMPANY_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `agentcompany run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use AgentCompany API
...
import { parseMyAgentStdoutLine } from "/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so AgentCompany wipes the stale session
...
Import from `/adapter-utils/server-utils`:
...
| `buildPaperclipEnv(agent)` | Standard `AGENTCOMPANY_*` env vars |
...
- Package name: `/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your AgentCompany work."`
...
### AgentCompany Skills Injection

AgentCompany ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `paperclip` API skill and the `paperclip-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with AgentCompany internals, break git status, and potentially leak into commits.
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(AGENTCOMPANY_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(AGENTCOMPANY_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks AgentCompany skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
- `AGENTCOMPANY_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the AgentCompany API and GitHub should not have access to arbitrary hosts.

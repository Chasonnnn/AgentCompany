...
describe("milestone approval", () => {
  ...
  test("worker cannot approve a milestone; emits policy.denied and leaves task unchanged", async () => {
    const dir = await mkTmpDir();
    await initWorkspace({ root_dir: dir, company_name: "Acme" });
    const { team_id } = await createTeam({ workspace_dir: dir, name: "Payments" });
    const worker = await createAgent({
    ...
    expect(fm.milestones.find((m) => m.id === ms.milestone_id)?.status).toBe("ready");
    ...
    expect(
      evs.some(
        (e) =>
          ...
          e.payload?.resource_id === report.artifact_id
      )
    ).toBe(true);
  });
});

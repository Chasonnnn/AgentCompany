...
const AGENTCOMPANY_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface AgentCompanySkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: AgentCompanySkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "agentcompany_required",
      originLabel: "Required by AgentCompany",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by AgentCompany",
    readOnly: false,
  };
}
...
type AgentCompanyWakePayload = {
  reason: string | null;
  issue: AgentCompanyWakeIssue | null;
  commentIds: string[];
  latestCommentId: string | null;
  comments: AgentCompanyWakeComment[];
...
function normalizeAgentCompanyWakeIssue(value: unknown): AgentCompanyWakeIssue | null {
...
function normalizeAgentCompanyWakeComment(value: unknown): AgentCompanyWakeComment | null {
...
export function normalizeAgentCompanyWakePayload(value: unknown): AgentCompanyWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeAgentCompanyWakeComment(entry))
        .filter((entry): entry is AgentCompanyWakeComment => Boolean(entry))
    : [];
  ...
  return {
    ...
    issue: normalizeAgentCompanyWakeIssue(payload.issue),
    commentIds,
...
export function stringifyAgentCompanyWakePayload(value: unknown): string | null {
  const normalized = normalizeAgentCompanyWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderAgentCompanyWakePrompt(
  value: unknown,
  ...
  const lines = resumedSession
      ? [
        "## AgentCompany Resume Delta",
        "",
        "You are resuming an existing AgentCompany session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "AGENTCOMPANY_RESOLVED_COMMAND"] = resolvedCommand;
  }
...
export function buildAgentCompanyEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.AGENTCOMPANY_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.AGENTCOMPANY_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.AGENTCOMPANY_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.AGENTCOMPANY_API_URL = apiUrl;
  return vars;
}
...
export async function resolveAgentCompanySkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...AGENTCOMPANY_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listAgentCompanySkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<AgentCompanySkillEntry[]> {
  const root = await resolveAgentCompanySkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `agentcompany/agentcompany/${entry.name}`,
        runtimeName: entry.name,
        source: path.join(root, entry.name),
        required: true,
        requiredReason: "Bundled AgentCompany skills are always available for local adapters.",
      }));
  } catch {
    return [];
  }
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the AgentCompany skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "AgentCompany cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredAgentCompanyRuntimeSkills(value: unknown): AgentCompanySkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: AgentCompanySkillEntry[] = [];
...
export async function readAgentCompanyRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<AgentCompanySkillEntry[]> {
  const configuredEntries = normalizeConfiguredAgentCompanyRuntimeSkills(config.agentcompanyRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listAgentCompanySkillEntries(moduleDir, additionalCandidates);
}

export async function readAgentCompanySkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listAgentCompanySkillEntries(moduleDir);
...
export function readAgentCompanySkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.agentcompanySkillSync;
...
function canonicalizeDesiredAgentCompanySkillReference(
  reference: string,
...
export function resolveAgentCompanyDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readAgentCompanySkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredAgentCompanySkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeAgentCompanySkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.agentcompanySkillSync;
  const current =
  ...
  next.agentcompanySkillSync = current;
  return next;
}

export async function ensureAgentCompanySkillSymlink(
  source: string,
  target: string,

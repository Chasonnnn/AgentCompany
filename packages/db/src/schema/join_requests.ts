...
export const joinRequests = pgTable(
  ...
  (table) => ({
    ...
    pendingHumanUserUniqueIdx: uniqueIndex("join_requests_pending_human_user_uq")
      ...
      .where(
        sql`${table.requestType} = 'human' AND ${table.status} = 'pending_approval' AND ${table.requestingUserId} IS NOT NULL`,
      ),
    pendingHumanEmailUniqueIdx: uniqueIndex("join_requests_pending_human_email_uq")
      ...
      .where(
        sql`${table.requestType} = 'human' AND ${table.status} = 'pending_approval' AND ${table.requestEmailSnapshot} IS NOT NULL`,
      ),
  }),
);

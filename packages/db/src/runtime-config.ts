...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "agentcompany-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveAgentCompanyHomeDir(), "instances", resolveAgentCompanyInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".agentcompany", CONFIG_BASENAME);
...
function resolveAgentCompanyConfigPath(): string {
  if (process.env.AGENTCOMPANY_CONFIG?.trim()) {
    return path.resolve(process.env.AGENTCOMPANY_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveAgentCompanyEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveAgentCompanyEnvPath(configPath);

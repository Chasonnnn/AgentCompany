...
function resolveAgentCompanyInstanceId(): string {
  const raw = process.env.AGENTCOMPANY_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveAgentCompanyHomeDir(), "instances", resolveAgentCompanyInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveAgentCompanyHomeDir(), "instances", resolveAgentCompanyInstanceId(), "data", "backups");
}

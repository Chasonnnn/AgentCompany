...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "agentcompany");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://agentcompany:agentcompany@127.0.0.1:${preferredPort}/agentcompany`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "agentcompany");
    return {
      connectionString: `postgres://agentcompany:agentcompany@127.0.0.1:${port}/agentcompany`,
  ...
  const adminConnectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "agentcompany");

  return {
    connectionString: `postgres://agentcompany:agentcompany@127.0.0.1:${selectedPort}/agentcompany`,

...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AGENTCOMPANY_OPENCODE_COMMAND === "string" &&
    process.env.AGENTCOMPANY_OPENCODE_COMMAND.trim().length > 0
      ? process.env.AGENTCOMPANY_OPENCODE_COMMAND.trim()
      : "opencode";

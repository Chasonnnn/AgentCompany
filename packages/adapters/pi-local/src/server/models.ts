...
import type { AdapterModel } from "@agentcompany/adapter-utils";
import { asString, runChildProcess } from "@agentcompany/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AGENTCOMPANY_PI_COMMAND === "string" &&
    process.env.AGENTCOMPANY_PI_COMMAND.trim().length > 0
      ? process.env.AGENTCOMPANY_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AGENTCOMPANY_", "npm_", "NPM_"] as const;

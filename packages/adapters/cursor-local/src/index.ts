...
Use when:
- You want AgentCompany to run Cursor Agent CLI locally as the agent runtime
...
- AgentCompany auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$agentcompany" and related skills on local runs.
- AgentCompany auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;

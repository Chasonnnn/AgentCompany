...
## Scope
This plan is now **gateway-only**. AgentCompany supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses AgentCompany skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a AgentCompany task.
...
```bash
pnpm --filter /server test:run -- openclaw-gateway
pnpm --filter /server typecheck
pnpm --filter /ui typecheck
pnpm --filter agentcompany typecheck
```

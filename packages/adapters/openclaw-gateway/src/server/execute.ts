...
const DEFAULT_CLIENT_VERSION = "agentcompany";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "agentcompany";
  if (input.strategy === "run") return `agentcompany:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `agentcompany:issue:${input.issueId}`;
  return fallback;
}
...
function resolveAgentCompanyApiUrlOverride(value: unknown): string | null {
...
function buildAgentCompanyEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const agentcompanyApiUrlOverride = resolveAgentCompanyApiUrlOverride(ctx.config.agentcompanyApiUrl);
  const agentcompanyEnv: Record<string, string> = {
    ...buildAgentCompanyEnv(ctx.agent),
    AGENTCOMPANY_RUN_ID: ctx.runId,
  };

  if (agentcompanyApiUrlOverride) {
    agentcompanyEnv.AGENTCOMPANY_API_URL = agentcompanyApiUrlOverride;
  }
  if (wakePayload.taskId) agentcompanyEnv.AGENTCOMPANY_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) agentcompanyEnv.AGENTCOMPANY_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) agentcompanyEnv.AGENTCOMPANY_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) agentcompanyEnv.AGENTCOMPANY_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) agentcompanyEnv.AGENTCOMPANY_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    agentcompanyEnv.AGENTCOMPANY_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return agentcompanyEnv;
}

function buildWakeText(
  payload: WakePayload,
  agentcompanyEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/agentcompany-claimed-api-key.json";
  const orderedKeys = [
    "AGENTCOMPANY_RUN_ID",
    "AGENTCOMPANY_AGENT_ID",
    "AGENTCOMPANY_COMPANY_ID",
    "AGENTCOMPANY_API_URL",
    "AGENTCOMPANY_TASK_ID",
    "AGENTCOMPANY_WAKE_REASON",
    "AGENTCOMPANY_WAKE_COMMENT_ID",
    "AGENTCOMPANY_APPROVAL_ID",
    "AGENTCOMPANY_APPROVAL_STATUS",
    "AGENTCOMPANY_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = agentcompanyEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = agentcompanyEnv.AGENTCOMPANY_API_URL ?? "<set AGENTCOMPANY_API_URL>";

  const lines = [
    "AgentCompany wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `AGENTCOMPANY_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load AGENTCOMPANY_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $AGENTCOMPANY_API_KEY on every API call.",
    "- Use X-AgentCompany-Run-Id: $AGENTCOMPANY_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: AGENTCOMPANY_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$AGENTCOMPANY_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$AGENTCOMPANY_COMPANY_ID/issues?assigneeAgentId=$AGENTCOMPANY_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardAgentCompanyPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  agentcompanyEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templatePaperclip = parseObject(payloadTemplate.agentcompany);
  const workspace = asRecord(ctx.context.agentcompanyWorkspace);
  const workspaces = Array.isArray(ctx.context.agentcompanyWorkspaces)
    ? ctx.context.agentcompanyWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.agentcompanyRuntimeServiceIntents)
    ? ctx.context.agentcompanyRuntimeServiceIntents.filter(
  ...
  const standardPaperclip: Record<string, unknown> = {
    ...
    apiUrl: agentcompanyEnv.AGENTCOMPANY_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.agentcompanyWake);
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "agentcompany-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const agentcompanyEnv = buildAgentCompanyEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderAgentCompanyWakePrompt(ctx.context.agentcompanyWake);
  const structuredWakeJson = stringifyAgentCompanyWakePayload(ctx.context.agentcompanyWake);
  ...
  const agentcompanyPayload = buildStandardAgentCompanyPayload(ctx, wakePayload, agentcompanyEnv, payloadTemplate);
  ...
  agentParams.agentcompany = agentcompanyPayload;

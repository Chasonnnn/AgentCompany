...
import { asString, parseObject } from "@agentcompany/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the AgentCompany server host.",
        });
      }
    } catch (err) {
      checks.push({

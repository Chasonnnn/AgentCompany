...
Use when:
- You want AgentCompany to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the AgentCompany server.
...
- agentcompanyApiUrl (string, optional): absolute AgentCompany base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default agentcompany)

Standard outbound payload additions:
- agentcompany (object): standardized AgentCompany context added to every gateway agent request
- agentcompany.workspace (object, optional): resolved execution workspace for this run
- agentcompany.workspaces (array, optional): additional workspace hints AgentCompany exposed to the run
- agentcompany.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution

...
This document describes how `/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into AgentCompany logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (AgentCompany `runId`)

...
Example AgentCompany plugin that demonstrates:
...
```bash
pnpm --filter /plugin-file-browser-example build
pnpm agentcompany plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm agentcompany plugin uninstall agentcompany-file-browser-example --force
```
...
- Optional: use `agentcompany-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.

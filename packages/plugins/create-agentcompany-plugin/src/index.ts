...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".agentcompany-sdk");
...
/**
 * Generate a complete AgentCompany plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A AgentCompany plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "agentcompany-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["agentcompany", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@agentcompany/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@agentcompany/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@agentcompany/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@agentcompany/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@agentcompany/plugin-sdk/ui";
...
import { createTestHarness } from "@agentcompany/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@agentcompany/plugin-sdk\` and \`@agentcompany/shared\` from a local AgentCompany checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.agentcompany-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@agentcompany/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-agentcompany-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <agentcompany-sdk-path>]");
    process.exit(1);
  }

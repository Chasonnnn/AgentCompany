...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
    ...
    return {
      ...
      issues: {
        ...
        async suggestTasks(issueId, interaction, companyId, options) {
          return callHost("issues.createInteraction", {
            issueId,
            interaction: { ...interaction, kind: "suggest_tasks" },
            companyId,
            authorAgentId: options?.authorAgentId,
          });
        },
  ...
  async function handleApiRequest(params: PluginApiRequestInput): Promise<PluginApiResponse> {
  ...
  async function handleEnvironmentValidateConfig(params: PluginEnvironmentValidateConfigParams) {

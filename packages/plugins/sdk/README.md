...
Official TypeScript SDK for AgentCompany plugin authors.

- **Worker SDK:** `/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `/plugin-sdk/ui/hooks` | Hooks only |
| `/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main AgentCompany app. They can call ordinary AgentCompany HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the AgentCompany instance at runtime.
...
```ts
import { definePlugin, runWorker } from "/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core AgentCompany information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "/plugin-sdk/ui";
import type { PluginDetailTabProps } from "/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "/plugin-sdk/bundlers";
...
```bash
agentcompany-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "/plugin-sdk/dev-server";

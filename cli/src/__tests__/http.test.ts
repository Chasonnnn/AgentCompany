...
import { ApiConnectionError, ApiRequestError, AgentCompanyApiClient } from "../client/http.js";

describe("AgentCompanyApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new AgentCompanyApiClient({
    ...
    expect(headers["x-agentcompany-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new AgentCompanyApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm agentcompany run/,
    );
  });

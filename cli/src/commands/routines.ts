...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "agentcompany");
      const connectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${embeddedHandle.port}/agentcompany`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? nonEmpty(process.env.AGENTCOMPANY_COMPANY_ID)
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set AGENTCOMPANY_COMPANY_ID.");
  }

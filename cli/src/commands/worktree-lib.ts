...
export const DEFAULT_WORKTREE_HOME = "~/.agentcompany-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".agentcompany");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: AgentCompanyConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "agentcompany",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    AGENTCOMPANY_HOME: paths.homeDir,
    AGENTCOMPANY_INSTANCE_ID: paths.instanceId,
    AGENTCOMPANY_CONFIG: paths.configPath,
    AGENTCOMPANY_CONTEXT: paths.contextPath,
    AGENTCOMPANY_IN_WORKTREE: "true",
    ...(branding?.name ? { AGENTCOMPANY_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { AGENTCOMPANY_WORKTREE_COLOR: branding.color } : {}),
  };
}

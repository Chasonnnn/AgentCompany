...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@agentcompany/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://agentcompany:agentcompany@127.0.0.1:${port}/agentcompany`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" agentcompany db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveAgentCompanyInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "agentcompany";

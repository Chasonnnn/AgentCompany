...
import { ensureAgentJwtSecret, loadAgentCompanyEnvFile, mergeAgentCompanyEnvEntries, readAgentCompanyEnvEntries, resolveAgentCompanyEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { AgentCompanyConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printAgentCompanyCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.AGENTCOMPANY_CONFIG;
...
const WORKTREE_NAME_PREFIX = "agentcompany-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.AGENTCOMPANY_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.AGENTCOMPANY_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromAgentCompanyConfig(config: AgentCompanyConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromAgentCompanyConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.agentcompany${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".agentcompany", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".agentcompany", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.agentcompany"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: AgentCompanyConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://agentcompany:agentcompany@127.0.0.1:${port}/agentcompany`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: AgentCompanyConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.AGENTCOMPANY_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AGENTCOMPANY_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.AGENTCOMPANY_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AGENTCOMPANY_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: AgentCompanyConfig;
  targetConfig: AgentCompanyConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveAgentCompanyEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readAgentCompanyEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "agentcompany");
    const targetConnectionString = `postgres://agentcompany:agentcompany@127.0.0.1:${targetHandle.port}/agentcompany`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readAgentCompanyEnvEntries(resolveAgentCompanyEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.AGENTCOMPANY_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.AGENTCOMPANY_AGENT_JWT_SECRET);
  mergeAgentCompanyEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { AGENTCOMPANY_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadAgentCompanyEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run AgentCompany inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printAgentCompanyCliBanner();
  p.intro(pc.bgCyan(pc.black(" agentcompany worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printAgentCompanyCliBanner();
  p.intro(pc.bgCyan(pc.black(" agentcompany worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasAgentCompanyConfig: existsSync(path.resolve(worktreePath, ".agentcompany", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printAgentCompanyCliBanner();
  p.intro(pc.bgCyan(pc.black(" agentcompany worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveAgentCompanyEnvFile(configPath);
  const envEntries = readAgentCompanyEnvEntries(envPath);
  const out = {
    AGENTCOMPANY_CONFIG: configPath,
    ...(envEntries.AGENTCOMPANY_HOME ? { AGENTCOMPANY_HOME: envEntries.AGENTCOMPANY_HOME } : {}),
    ...(envEntries.AGENTCOMPANY_INSTANCE_ID ? { AGENTCOMPANY_INSTANCE_ID: envEntries.AGENTCOMPANY_INSTANCE_ID } : {}),
    ...(envEntries.AGENTCOMPANY_CONTEXT ? { AGENTCOMPANY_CONTEXT: envEntries.AGENTCOMPANY_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasAgentCompanyConfig)
      .map((choice) => path.resolve(choice.worktree, ".agentcompany", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readAgentCompanyEnvEntries(resolveAgentCompanyEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start AgentCompany once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasAgentCompanyConfig ? "agentcompany" : "no-agentcompany-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".agentcompany", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".agentcompany", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .agentcompany/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasAgentCompanyConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a AgentCompany worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasAgentCompanyConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No AgentCompany worktrees were found. Run `agentcompany worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target AgentCompany configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local AgentCompany instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated AgentCompany instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with agentcompany- if needed (created at ~/agentcompany-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: AGENTCOMPANY_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: AGENTCOMPANY_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source AGENTCOMPANY_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local AgentCompany instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like AgentCompany worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with agentcompany- if needed")

...
type OnboardDefaults = Pick<AgentCompanyConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "AGENTCOMPANY_PUBLIC_URL",
  "DATABASE_URL",
  "AGENTCOMPANY_DB_BACKUP_ENABLED",
  "AGENTCOMPANY_DB_BACKUP_INTERVAL_MINUTES",
  "AGENTCOMPANY_DB_BACKUP_RETENTION_DAYS",
  "AGENTCOMPANY_DB_BACKUP_DIR",
  "AGENTCOMPANY_DEPLOYMENT_MODE",
  "AGENTCOMPANY_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "AGENTCOMPANY_ALLOWED_HOSTNAMES",
  "AGENTCOMPANY_AUTH_BASE_URL_MODE",
  "AGENTCOMPANY_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "AGENTCOMPANY_STORAGE_PROVIDER",
  "AGENTCOMPANY_STORAGE_LOCAL_DIR",
  "AGENTCOMPANY_STORAGE_S3_BUCKET",
  "AGENTCOMPANY_STORAGE_S3_REGION",
  "AGENTCOMPANY_STORAGE_S3_ENDPOINT",
  "AGENTCOMPANY_STORAGE_S3_PREFIX",
  "AGENTCOMPANY_STORAGE_S3_FORCE_PATH_STYLE",
  "AGENTCOMPANY_SECRETS_PROVIDER",
  "AGENTCOMPANY_SECRETS_STRICT_MODE",
  "AGENTCOMPANY_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveAgentCompanyInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.AGENTCOMPANY_PUBLIC_URL?.trim() ||
    process.env.AGENTCOMPANY_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.AGENTCOMPANY_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.AGENTCOMPANY_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.AGENTCOMPANY_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.AGENTCOMPANY_ALLOWED_HOSTNAMES
    ? process.env.AGENTCOMPANY_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.AGENTCOMPANY_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.AGENTCOMPANY_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.AGENTCOMPANY_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.AGENTCOMPANY_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.AGENTCOMPANY_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.AGENTCOMPANY_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.AGENTCOMPANY_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.AGENTCOMPANY_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.AGENTCOMPANY_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.AGENTCOMPANY_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.AGENTCOMPANY_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.AGENTCOMPANY_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.AGENTCOMPANY_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.AGENTCOMPANY_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.AGENTCOMPANY_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "AGENTCOMPANY_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<AgentCompanyConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printAgentCompanyCliBanner();
  p.intro(pc.bgCyan(pc.black(" agentcompany onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveAgentCompanyInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: AgentCompanyConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing AgentCompany install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("agentcompany configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("AGENTCOMPANY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.AGENTCOMPANY_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("AGENTCOMPANY_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("AGENTCOMPANY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: AGENTCOMPANY_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("agentcompany run")}`,
        `Reconfigure later: ${pc.cyan("agentcompany configure")}`,
        `Diagnose setup: ${pc.cyan("agentcompany doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (!shouldRunNow && !opts.invokedByRun && process.stdin.isTTY && process.stdout.isTTY) {
      const answer = await p.confirm({
        message: "Start AgentCompany now?",
        initialValue: true,
      });
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.AGENTCOMPANY_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing AgentCompany setup is ready.");
    return;
  }
  ...
  let llm: AgentCompanyConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@agentcompany/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `agentcompany doctor`"));
      }
    }
  ...
  const config: AgentCompanyConfig = {
    $meta: {
      version: 1,
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("agentcompany run")}`,
        `Then: ${pc.cyan("agentcompany auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }

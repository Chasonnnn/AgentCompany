...
import { ApiRequestError, AgentCompanyApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to AgentCompany config file")
    .option("-d, --data-dir <path>", "AgentCompany data directory root (isolates state from ~/.agentcompany)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the AgentCompany API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AGENTCOMPANY_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.AGENTCOMPANY_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.AGENTCOMPANY_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AGENTCOMPANY_COMPANY_ID, or set context profile companyId via `agentcompany context set`.",
    );
  }

  const api = new AgentCompanyApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AGENTCOMPANY_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AGENTCOMPANY_SERVER_PORT || "");

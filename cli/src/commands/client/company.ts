...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isAgentCompanyYaml = baseName === ".agentcompany.yaml" || baseName === ".agentcompany.yml";
  const contentType = binaryContentTypeByExtension[path.extname(baseName).toLowerCase()];
  return isMarkdown || isAgentCompanyYaml || Boolean(contentType);
}

function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".agentcompany.yaml"] !== undefined) return ".agentcompany.yaml";
  if (files[".agentcompany.yml"] !== undefined) return ".agentcompany.yml";
...
async function promptForImportSelection(preview: CompanyPortabilityPreviewResult): Promise<string[]> {
  ...
  while (true) {
    const choice = await p.select<ImportSelectableGroup | "company" | "confirm">({
      message: "Select what AgentCompany should import",
      options: [
        {
          value: "company",
...
export function registerCompanyCommands(program: Command): void {
          ...
          printOutput(
            {
              ...
              agentcompanyExtensionPath: exported.agentcompanyExtensionPath,
  ...
  addCommonClientOptions(
    company
      ...
      .option("--agentcompany-url <url>", "Alias for --api-base on this command")
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          if (!opts.apiBase?.trim() && opts.agentcompanyUrl?.trim()) {
            opts.apiBase = opts.agentcompanyUrl.trim();
          }
          const ctx = resolveCommandContext(opts);

...
import type { Agent } from "@agentcompany/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveAgentCompanySkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export AGENTCOMPANY_API_URL='${escaped(input.apiBase)}'`,
    `export AGENTCOMPANY_COMPANY_ID='${escaped(input.companyId)}'`,
    `export AGENTCOMPANY_AGENT_ID='${escaped(input.agentId)}'`,
    `export AGENTCOMPANY_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local AgentCompany skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing AgentCompany skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveAgentCompanySkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local AgentCompany skills directory. Expected ./skills in the repo checkout.",
              );
            }

...
export class AgentCompanyApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-agentcompany-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the AgentCompany API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the AgentCompany server is not running, the configured URL is wrong, or the request is being blocked before it reaches AgentCompany.",
    "",
    "Try:",
    "- Start AgentCompany with `pnpm dev` or `pnpm agentcompany run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If AgentCompany is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`AGENTCOMPANY_API_URL\`.`,
  );
  return lines.join("\n");
}

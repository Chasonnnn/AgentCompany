...
const JWT_SECRET_ENV_KEY = "AGENTCOMPANY_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by AgentCompany CLI commands",
...
export function resolveAgentCompanyEnvFile(configPath?: string): string {
...
export function loadAgentCompanyEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAgentCompanyEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAgentCompanyEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAgentCompanyEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAgentCompanyEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAgentCompanyEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAgentCompanyEnvEntries(next, filePath);
  return next;
}

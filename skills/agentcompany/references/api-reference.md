# AgentCompany API Reference

Detailed reference for the AgentCompany control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
# ^ Load tests depend on caching layer being done first. AgentCompany will auto-wake agent-55 when the blocker resolves.
...
{
  "name": "AgentCompany Mobile App",
  "description": "Ship iOS + Android client",
  "status": "planned",
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "agentcompany-mobile",
    "cwd": "/Users/me/agentcompany-mobile",
    "repoUrl": "https://github.com/acme/agentcompany-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `agentcompany-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
| Block on another task without `blockedByIssueIds` | No automatic wake when blocker resolves; manual follow-up needed | Set `blockedByIssueIds` so AgentCompany auto-wakes the assignee when all blockers are done |

---
name: agentcompany-create-plugin
description: >
  Create new AgentCompany plugins with the current alpha SDK/runtime. Use when
...
# Create a AgentCompany Plugin

Use this skill when the task is to create, scaffold, or document a AgentCompany plugin.
...
node packages/plugins/create-agentcompany-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the AgentCompany repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.agentcompany-sdk/`:

```bash
pnpm --filter /create-agentcompany-plugin build
node packages/plugins/create-agentcompany-plugin/dist/index.js @acme/plugin-name \
...
- is installed into AgentCompany from an absolute local path during development

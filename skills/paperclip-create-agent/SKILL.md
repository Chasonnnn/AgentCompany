...
- capabilities
- managed instructions bundle (`AGENTS.md`) for adapters that support it; avoid durable `promptTemplate` config for role instructions
...
- instruction text such as `AGENTS.md` built from step 4; for local managed-bundle adapters, send it in top-level `instructionsBundle.files["AGENTS.md"]`. Do not set `adapterConfig.promptTemplate` or `adapterConfig.bootstrapPromptTemplate` for new agents unless the adapter explicitly lacks managed bundle support.

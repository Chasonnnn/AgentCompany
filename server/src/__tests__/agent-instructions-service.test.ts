...
describe("agent instructions service", () => {
  const originalAgentCompanyHome = process.env.AGENTCOMPANY_HOME;
  const originalAgentCompanyInstanceId = process.env.AGENTCOMPANY_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalAgentCompanyHome === undefined) delete process.env.AGENTCOMPANY_HOME;
    else process.env.AGENTCOMPANY_HOME = originalAgentCompanyHome;
    if (originalAgentCompanyInstanceId === undefined) delete process.env.AGENTCOMPANY_INSTANCE_ID;
    else process.env.AGENTCOMPANY_INSTANCE_ID = originalAgentCompanyInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const agentcompanyHome = await makeTempDir("paperclip-agent-instructions-home-");
    const externalRoot = await makeTempDir("paperclip-agent-instructions-external-");
    cleanupDirs.add(agentcompanyHome);
    cleanupDirs.add(externalRoot);
    process.env.AGENTCOMPANY_HOME = agentcompanyHome;
    process.env.AGENTCOMPANY_INSTANCE_ID = "test-instance";
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const agentcompanyHome = await makeTempDir("paperclip-agent-instructions-recover-");
    cleanupDirs.add(agentcompanyHome);
    process.env.AGENTCOMPANY_HOME = agentcompanyHome;
    process.env.AGENTCOMPANY_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const agentcompanyHome = await makeTempDir("paperclip-agent-instructions-stale-managed-");
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const agentcompanyHome = await makeTempDir("paperclip-agent-instructions-heal-write-");
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const agentcompanyHome = await makeTempDir("paperclip-agent-instructions-heal-delete-");
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const agentcompanyHome = await makeTempDir("paperclip-agent-instructions-partial-managed-");

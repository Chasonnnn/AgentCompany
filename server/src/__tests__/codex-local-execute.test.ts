...
import { execute } from "@agentcompany/adapter-codex-local/server";

async function writeFakeCodexCommand(commandPath: string): Promise<void> {
  const script = `#!/usr/bin/env node
const fs = require("node:fs");

const capturePath = process.env.AGENTCOMPANY_TEST_CAPTURE_PATH;
const payload = {
  argv: process.argv.slice(2),
  prompt: fs.readFileSync(0, "utf8"),
  codexHome: process.env.CODEX_HOME || null,
  agentcompanyWakePayloadJson: process.env.AGENTCOMPANY_WAKE_PAYLOAD_JSON || null,
  agentcompanyEnvKeys: Object.keys(process.env)
...
type CapturePayload = {
  ...
  agentcompanyWakePayloadJson: string | null;
  agentcompanyEnvKeys: string[];
};
...
describe("codex execute", () => {
  it("uses a AgentCompany-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-execute-default-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const agentcompanyHome = path.join(root, "paperclip-home");
    ...
    const previousAgentCompanyHome = process.env.AGENTCOMPANY_HOME;
    const previousAgentCompanyInstanceId = process.env.AGENTCOMPANY_INSTANCE_ID;
    const previousAgentCompanyInWorktree = process.env.AGENTCOMPANY_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.AGENTCOMPANY_HOME = agentcompanyHome;
    delete process.env.AGENTCOMPANY_INSTANCE_ID;
    delete process.env.AGENTCOMPANY_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            AGENTCOMPANY_TEST_CAPTURE_PATH: capturePath,
          },
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using AgentCompany-managed Codex home"),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousAgentCompanyHome === undefined) delete process.env.AGENTCOMPANY_HOME;
      else process.env.AGENTCOMPANY_HOME = previousAgentCompanyHome;
      if (previousAgentCompanyInstanceId === undefined) delete process.env.AGENTCOMPANY_INSTANCE_ID;
      else process.env.AGENTCOMPANY_INSTANCE_ID = previousAgentCompanyInstanceId;
      if (previousAgentCompanyInWorktree === undefined) delete process.env.AGENTCOMPANY_IN_WORKTREE;
      else process.env.AGENTCOMPANY_IN_WORKTREE = previousAgentCompanyInWorktree;
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    ...
    try {
      ...
      expect(loggedEnv.AGENTCOMPANY_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects structured AgentCompany wake payloads into env and prompt", async () => {
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.agentcompanyEnvKeys).toContain("AGENTCOMPANY_WAKE_PAYLOAD_JSON");
      expect(capture.agentcompanyWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.agentcompanyWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## AgentCompany Wake Payload");
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    ...
    try {
      ...
      expect(capture.prompt).toContain("## AgentCompany Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      ...
      expect(invocationPrompt).toContain("## AgentCompany Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    ...
    process.env.HOME = root;
    process.env.AGENTCOMPANY_HOME = agentcompanyHome;
    process.env.AGENTCOMPANY_INSTANCE_ID = "worktree-1";
    process.env.AGENTCOMPANY_IN_WORKTREE = "true";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      expect(capture.agentcompanyEnvKeys).toEqual(
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    ...
    try {
      ...
      await expect(fs.lstat(path.join(agentcompanyHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousAgentCompanyHome === undefined) delete process.env.AGENTCOMPANY_HOME;

...
import { ensureCodexSkillsInjected } from "@agentcompany/adapter-codex-local/server";

async function makeTempDir(prefix: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), prefix));
}

async function createAgentCompanyRepoSkill(root: string, skillName: string) {
...
describe("codex local adapter skill injection", () => {
  ...
  it("repairs a Codex AgentCompany skill symlink that still points at another live checkout", async () => {
    ...
    await createAgentCompanyRepoSkill(currentRepo, "paperclip");
    await createAgentCompanyRepoSkill(oldRepo, "paperclip");
  ...
  it("preserves a custom Codex skill symlink outside AgentCompany repo checkouts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
  ...
  it("prunes broken symlinks for unavailable AgentCompany repo skills before Codex starts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const oldRepo = await makeTempDir("paperclip-codex-old-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
    ...
    await createAgentCompanyRepoSkill(currentRepo, "paperclip");
    await createAgentCompanyRepoSkill(oldRepo, "agent-browser");
  ...
  it("preserves other live AgentCompany skill symlinks in the shared workspace skill directory", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
    cleanupDirs.add(skillsHome);
    ...
    await createAgentCompanyRepoSkill(currentRepo, "paperclip");
    await createAgentCompanyRepoSkill(currentRepo, "agent-browser");
    await fs.symlink(

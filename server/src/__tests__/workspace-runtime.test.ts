...
import { resolveAgentCompanyConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@agentcompany/shared";
...
async function createTempRepo(defaultBranch = "main") {
  ...
  await runGit(repoRoot, ["config", "user.name", "AgentCompany Test"]);
...
afterEach(async () => {
  ...
  delete process.env.AGENTCOMPANY_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited AgentCompany and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/paperclip",
      AGENTCOMPANY_HOME: "/tmp/paperclip-home",
      AGENTCOMPANY_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.AGENTCOMPANY_HOME).toBeUndefined();
    expect(sanitized.AGENTCOMPANY_INSTANCE_ID).toBeUndefined();
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".agentcompany", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$AGENTCOMPANY_WORKSPACE_BRANCH\" > .agentcompany-provision-branch",
        "printf '%s\\n' \"$AGENTCOMPANY_WORKSPACE_BASE_CWD\" > .agentcompany-provision-base",
        "printf '%s\\n' \"$AGENTCOMPANY_WORKSPACE_CREATED\" > .agentcompany-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".agentcompany-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".agentcompany-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".agentcompany-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".agentcompany-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local AgentCompany config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const agentcompanyHome = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(agentcompanyHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.AGENTCOMPANY_HOME = agentcompanyHome;
    process.env.AGENTCOMPANY_INSTANCE_ID = instanceId;
    process.env.AGENTCOMPANY_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".agentcompany", "config.json");
      const envPath = path.join(workspace.cwd, ".agentcompany", ".env");
      ...
      expect(envVars.AGENTCOMPANY_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.AGENTCOMPANY_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.AGENTCOMPANY_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.AGENTCOMPANY_IN_WORKTREE).toBe("true");
      expect(envVars.AGENTCOMPANY_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveAgentCompanyConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    ...
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".agentcompany", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.AGENTCOMPANY_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              AGENTCOMPANY_HOME: "{{workspace.cwd}}/.agentcompany/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".agentcompany", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".agentcompany", "runtime-services"));
  });

  it("does not leak parent AgentCompany instance env into runtime service commands", async () => {
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "agentcompanyConfig: process.env.AGENTCOMPANY_CONFIG ?? null,",
          "agentcompanyHome: process.env.AGENTCOMPANY_HOME ?? null,",
          "agentcompanyInstanceId: process.env.AGENTCOMPANY_INSTANCE_ID ?? null,",
    ...
    process.env.AGENTCOMPANY_CONFIG = "/tmp/base-paperclip-config.json";
    process.env.AGENTCOMPANY_HOME = "/tmp/base-paperclip-home";
    process.env.AGENTCOMPANY_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres:/@agentcompany/shared-db.example.com/paperclip";
    ...
    expect(captured.agentcompanyConfig).toBeNull();
    expect(captured.agentcompanyHome).toBeNull();
    expect(captured.agentcompanyInstanceId).toBeNull();
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    ...
    const agentcompanyHome = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-runtime-home-"));
    process.env.AGENTCOMPANY_HOME = agentcompanyHome;
    process.env.AGENTCOMPANY_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;

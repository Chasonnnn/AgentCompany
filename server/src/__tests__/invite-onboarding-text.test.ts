...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("AgentCompany OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested AgentCompany base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("agentcompanyApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.agentcompanyApiUrl");
    expect(text).toContain("~/.openclaw/workspace/paperclip-claimed-api-key.json");
    expect(text).toContain("AGENTCOMPANY_API_KEY");

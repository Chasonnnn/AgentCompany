...
import { testEnvironment } from "@agentcompany/adapter-gemini-local/server";
...
const outPath = process.env.AGENTCOMPANY_TEST_ARGS_PATH;
if (outPath) {
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          AGENTCOMPANY_TEST_ARGS_PATH: argsCapturePath,

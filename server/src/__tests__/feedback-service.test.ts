...
import { writeAgentCompanySkillSyncPreference } from "@agentcompany/adapter-utils/server-utils";
import {
  agents,
...
describe("feedbackService.saveIssueVote", () => {
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    await db.insert(companySkills).values([
      {
        id: randomUUID(),
        companyId,
        key: "agentcompany/agentcompany/paperclip",
        slug: "paperclip",
        name: "AgentCompany",
        markdown: "# AgentCompany",
        sourceType: "catalog",
        sourceLocator: null,
        sourceRef: null,
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeAgentCompanySkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["agentcompany/agentcompany/paperclip", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    ...
    vi.stubEnv("AGENTCOMPANY_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,

...
import type { CompanyPortabilityFileEntry } from "@agentcompany/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "AgentCompany coordination skill",
        markdown: "---\nname: paperclip\ndescription: AgentCompany coordination skill\n---\n\n# AgentCompany\n",
        sourceType: "github",
        sourceLocator: "https://github.com/chason/agentcompany/tree/master/skills/paperclip",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/agentcompany/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "agentcompany",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/agentcompany/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "agentcompany",
  ...
  it("exports referenced skills as stubs by default with sanitized AgentCompany extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "AgentCompany"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${agentcompanyKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/paperclip/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/agentcompany/agentcompany/paperclip/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".agentcompany.yaml"]);
    ...
    expect(extension).not.toContain("agentcompanySkillSync");
  ...
  it("exports default sidebar order into the AgentCompany extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".agentcompany.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/paperclip/SKILL.md"])).toContain("# AgentCompany");
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/paperclip/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/agentcompany/agentcompany/paperclip/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/agentcompany/agentcompany/paperclip/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .agentcompany.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".agentcompany.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-paperclip",
        companyId: "company-1",
        key: "agentcompany/agentcompany/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/chason/agentcompany/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "agentcompany_bundled",
          owner: "agentcompany",
          repo: "paperclip",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/agentcompany/agentcompany/release-changelog/SKILL.md"])).toContain("agentcompany/agentcompany/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            repoUrl: "https://github.com/chason/agentcompany.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .agentcompany-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/chason/agentcompany.git"');
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported AgentCompany",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "https://github.com/chason/agentcompany.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
  ...
  it("reads env inputs back from .agentcompany.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with AgentCompany routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      ...
      target: { mode: "new_company", newCompanyName: "Imported AgentCompany" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported AgentCompany"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .agentcompany.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported AgentCompany");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".agentcompany.yaml"] = `${exported.files[".agentcompany.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',

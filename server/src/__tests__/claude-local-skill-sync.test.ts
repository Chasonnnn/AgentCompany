...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "agentcompany/agentcompany/paperclip-create-agent";
  ...
  it("defaults to mounting all built-in AgentCompany skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === agentcompanyKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat AgentCompany skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",

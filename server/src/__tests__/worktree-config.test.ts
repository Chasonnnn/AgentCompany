...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    ...
    const agentcompanyDir = path.join(worktreeRoot, ".agentcompany");
    const configPath = path.join(agentcompanyDir, "config.json");
    const envPath = path.join(agentcompanyDir, ".env");
    const sharedRoot = path.join(tempRoot, ".agentcompany", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".agentcompany-worktrees");

    await fs.mkdir(agentcompanyDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        ...
        "AGENTCOMPANY_WORKTREE_NAME=PAP-884-ai-commits-component",
        "AGENTCOMPANY_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.AGENTCOMPANY_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.AGENTCOMPANY_WORKTREES_DIR = isolatedHome;
    delete process.env.AGENTCOMPANY_HOME;
    delete process.env.AGENTCOMPANY_INSTANCE_ID;
    delete process.env.AGENTCOMPANY_CONFIG;
    delete process.env.AGENTCOMPANY_CONTEXT;
    ...
    expect(repairedEnv).toContain(`AGENTCOMPANY_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('AGENTCOMPANY_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`AGENTCOMPANY_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`AGENTCOMPANY_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('AGENTCOMPANY_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.AGENTCOMPANY_HOME).toBe(isolatedHome);
    expect(process.env.AGENTCOMPANY_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });
  ...
  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "# AgentCompany environment variables",
        "AGENTCOMPANY_IN_WORKTREE=true",
        "AGENTCOMPANY_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.AGENTCOMPANY_IN_WORKTREE = "true";
    process.env.AGENTCOMPANY_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.AGENTCOMPANY_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    ...
    const isolatedHome = path.join(tempRoot, ".agentcompany-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".agentcompany", "worktrees");
    ...
    const agentcompanyDir = path.join(currentWorktreeRoot, ".agentcompany");
    const configPath = path.join(agentcompanyDir, "config.json");
    const envPath = path.join(agentcompanyDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".agentcompany", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    ...
    process.chdir(worktreeRoot);
    process.env.AGENTCOMPANY_IN_WORKTREE = "true";
    process.env.AGENTCOMPANY_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.AGENTCOMPANY_HOME = isolatedHome;
    process.env.AGENTCOMPANY_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.AGENTCOMPANY_CONFIG = configPath;

...
import { execute, testEnvironment } from "@agentcompany/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@agentcompany/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              agentcompanyWorkspaces: [
                {
                  id: "workspace-1",
                  cwd: "/tmp/project",
                },
              ],
              agentcompanyRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(String(payload?.message ?? "")).toContain("AGENTCOMPANY_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("AGENTCOMPANY_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## AgentCompany Wake Payload");
      ...
      expect(payload?.agentcompany).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },

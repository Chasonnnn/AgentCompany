...
const mockIssueContinuityService = vi.hoisted(() => ({
  recomputeIssueContinuityState: vi.fn(),
  getIssueContinuity: vi.fn(),
  prepare: vi.fn(),
  requestPlanApproval: vi.fn(),
  handoff: vi.fn(),
  progressCheckpoint: vi.fn(),
  reviewReturn: vi.fn(),
...
describe("agent issue mutation checkout ownership", () => {
  beforeEach(() => {
    ...
    mockIssueContinuityService.getIssueContinuity.mockResolvedValue({
      continuityState: null,
      continuitySummary: null,
      decisionQuestions: [],
      approvals: [],
    });
    mockIssueContinuityService.prepare.mockResolvedValue({});
    mockIssueContinuityService.requestPlanApproval.mockResolvedValue({});
    mockIssueContinuityService.handoff.mockResolvedValue({});
    mockIssueContinuityService.progressCheckpoint.mockResolvedValue({});
    mockIssueContinuityService.reviewReturn.mockResolvedValue({});
    mockIssueContinuityService.reviewResubmit.mockResolvedValue({});
    mockIssueContinuityService.handoffRepair.mockResolvedValue({});
    mockIssueContinuityService.handoffCancel.mockResolvedValue({});
    mockIssueContinuityService.requestSpecThaw.mockResolvedValue({});
    mockIssueContinuityService.mutateBranch.mockResolvedValue({});
  ...
  it("keeps same-company peers from directly mutating todo issues they do not own", async () => {
  ...
  it("keeps same-company peers from directly mutating unassigned in-progress issues", async () => {

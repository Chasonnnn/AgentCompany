...
import type { AgentCompanyConfig } from "@agentcompany/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by AgentCompany worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.AGENTCOMPANY_IN_WORKTREE !== "true") return null;

  const configPath = resolveAgentCompanyConfigPath(overrideConfigPath);
  const envPath = resolveAgentCompanyEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.AGENTCOMPANY_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.AGENTCOMPANY_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.AGENTCOMPANY_HOME) ??
      nonEmpty(env.AGENTCOMPANY_WORKTREES_DIR) ??
      "~/.agentcompany-worktrees",
  );
...
function writeConfigFile(configPath: string, config: AgentCompanyConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".agentcompany", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as AgentCompanyConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: AgentCompanyConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: AgentCompanyConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.AGENTCOMPANY_HOME = context.homeDir;
  process.env.AGENTCOMPANY_INSTANCE_ID = context.instanceId;
  process.env.AGENTCOMPANY_CONFIG = context.configPath;
  process.env.AGENTCOMPANY_CONTEXT = context.contextPath;
  process.env.AGENTCOMPANY_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgentCompanyConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    AGENTCOMPANY_HOME: context.homeDir,
    AGENTCOMPANY_INSTANCE_ID: context.instanceId,
    AGENTCOMPANY_CONFIG: context.configPath,
    AGENTCOMPANY_CONTEXT: context.contextPath,
    AGENTCOMPANY_IN_WORKTREE: "true",
    AGENTCOMPANY_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: AgentCompanyConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as AgentCompanyConfig;
  } catch {
    return;
  }

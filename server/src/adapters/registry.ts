...
import { getAdapterSessionManagement } from "@agentcompany/adapter-utils";
import {
...
} from "@agentcompany/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@agentcompany/adapter-claude-local";
import {
...
} from "@agentcompany/adapter-codex-local/server";
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@agentcompany/adapter-codex-local";
import {
...
} from "@agentcompany/adapter-cursor-local/server";
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@agentcompany/adapter-cursor-local";
import {
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@agentcompany/adapter-gemini-local";
import {
  ...
  listOpenCodeModels,
} from "@agentcompany/adapter-opencode-local/server";
import {
  agentConfigurationDoc as openCodeAgentConfigurationDoc,
  models as openCodeModels,
} from "@agentcompany/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@agentcompany/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@agentcompany/adapter-openclaw-gateway";
...
} from "@agentcompany/adapter-pi-local";
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[agentcompany] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[agentcompany] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[agentcompany] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[agentcompany] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}

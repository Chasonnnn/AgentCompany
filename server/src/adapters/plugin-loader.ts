...
function extractUiParserSource(
  ...
  const contractVersion = pkg.agentcompany?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no legacy paperclip.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }

const FAVICON_BLOCK_START = "<!-- AGENTCOMPANY_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- AGENTCOMPANY_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- AGENTCOMPANY_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- AGENTCOMPANY_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.AGENTCOMPANY_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.AGENTCOMPANY_WORKTREE_NAME) ?? nonEmpty(env.AGENTCOMPANY_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.AGENTCOMPANY_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="agentcompany-worktree-enabled" content="true" />',
    `<meta name="agentcompany-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="agentcompany-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="agentcompany-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}

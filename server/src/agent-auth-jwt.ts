...
function jwtConfig() {
  const secret = process.env.AGENTCOMPANY_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.AGENTCOMPANY_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.AGENTCOMPANY_AGENT_JWT_ISSUER ?? "agentcompany",
    audience: process.env.AGENTCOMPANY_AGENT_JWT_AUDIENCE ?? "agentcompany-api",
  };
}

...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@agentcompany/db";
...
import { trackAgentCreated } from "@agentcompany/shared/telemetry";
...
import { runClaudeLogin } from "@agentcompany/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@agentcompany/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@agentcompany/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@agentcompany/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@agentcompany/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.AGENTCOMPANY_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeAgentCompanySkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readAgentCompanySkillSyncPreference(

...
interface AdapterInstallRequest {
  /** npm package name (e.g., "@acme/agentcompany-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@acme/agentcompany-adapter@0.3.0" → packageName + version
  ...
  // without modifying AgentCompany's source code.
  //

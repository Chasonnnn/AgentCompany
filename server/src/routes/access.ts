...
import type { DeploymentExposure, DeploymentMode } from "@agentcompany/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "agentcompany" &&
    normalized !== "agentcompany-create-agent" &&
    normalized !== "agentcompany-create-plugin" &&
...
/** Resolve the AgentCompany repo skills directory (built-in / managed skills). */
function resolveAgentCompanySkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const agentcompanySkillsDir = resolveAgentCompanySkillsDir();

  // Build set of AgentCompany-managed skill names
  const agentcompanySkillNames = new Set<string>();
  if (agentcompanySkillsDir) {
    try {
      for (const entry of fs.readdirSync(agentcompanySkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) agentcompanySkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isAgentCompanyManaged: agentcompanySkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.agentcompanyApiUrl)) {
    const legacyAgentCompanyApiUrl = nonEmptyTrimmedString(input.agentcompanyApiUrl);
    if (legacyAgentCompanyApiUrl) merged.agentcompanyApiUrl = legacyAgentCompanyApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    agentcompanyApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.agentcompanyApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawAgentCompanyApiUrl =
    typeof defaults.agentcompanyApiUrl === "string"
      ? defaults.agentcompanyApiUrl.trim()
      : "";
  if (rawAgentCompanyApiUrl) {
    try {
      const parsedAgentCompanyApiUrl = new URL(rawAgentCompanyApiUrl);
      if (
        parsedAgentCompanyApiUrl.protocol !== "http:" &&
        parsedAgentCompanyApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_agentcompany_api_url_protocol",
          level: "warn",
          message: `agentcompanyApiUrl must use http:// or https:// (got ${parsedAgentCompanyApiUrl.protocol}).`
        });
      } else {
        normalized.agentcompanyApiUrl = parsedAgentCompanyApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_agentcompany_api_url_configured",
          level: "info",
          message: `agentcompanyApiUrl set to ${parsedAgentCompanyApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_agentcompany_api_url_invalid",
        level: "warn",
        message: `Invalid agentcompanyApiUrl: ${rawAgentCompanyApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your AgentCompany host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "AgentCompany is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm agentcompany allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/agentcompany";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/agentcompany-claimed-api-key.json and load AGENTCOMPANY_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: agentcompanyApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the AgentCompany hostname is reachable and allowed via `pnpm agentcompany allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this AgentCompany API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/agentcompany/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # AgentCompany OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          agentcompanyApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, AgentCompany generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "agentcompanyApiUrl": "https://agentcompany-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in AgentCompany before key claim is allowed.
    ...
    - ~/.openclaw/workspace/agentcompany-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/agentcompany-claimed-api-key.json

    And set the AGENTCOMPANY_API_KEY and AGENTCOMPANY_API_URL in your environment variables as specified here:
    ...
    {
      env: {
        AGENTCOMPANY_API_KEY: "...",
        AGENTCOMPANY_API_URL: "...",
      },
    }

    Then set AGENTCOMPANY_API_KEY and AGENTCOMPANY_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install AgentCompany skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your AGENTCOMPANY_API_URL to the top of your skill and note the path to your AGENTCOMPANY_API_URL
    ...
    ${
      ...
      "Ensure AgentCompany is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested AgentCompany base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.agentcompanyApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm agentcompany allowed-hostname <host>
      - then restart AgentCompany and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@agentcompany.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "agentcompany", path: "/api/skills/agentcompany" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "agentcompany-create-agent",
          path: "/api/skills/agentcompany-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              agentcompanyApiUrl: req.body.agentcompanyApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.agentcompanyApiUrl &&
          !persistedDefaults.agentcompanyApiUrl
        ) {
          missingPersistedFields.push("agentcompanyApiUrl");
        }

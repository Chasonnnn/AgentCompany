...
import { issues, projects, projectWorkspaces } from "@agentcompany/db";
import { updateExecutionWorkspaceSchema } from "@agentcompany/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before AgentCompany can manage local runtime services" });
      return;
    }

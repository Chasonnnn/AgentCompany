 ...
 *    (`~/.agentcompany/plugins/`) and `node_modules` for packages matching
 *    the `agentcompany-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published AgentCompany plugins.
 * Packages matching this pattern are considered AgentCompany plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "agentcompany-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: AgentCompanyPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.agentcompany/plugins/ local directory
  | "npm"               // npm packages matching agentcompany-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: AgentCompanyPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.agentcompany/plugins/
   */
  ...
  /**
   * Whether to discover installed npm packages matching the agentcompany-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "agentcompany-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: AgentCompanyPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover AgentCompany plugins installed as npm packages in the current
   * Node.js environment matching the "agentcompany-plugin-*" naming convention.
   *
   ...
   * the "agentcompanyPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a AgentCompany plugin.
   * Throws if the package is a AgentCompany plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<AgentCompanyPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: AgentCompanyPluginManifestV1;
    newManifest: AgentCompanyPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the AgentCompany plugin naming convention.
 * Accepts both the "agentcompany-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or /plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "agentcompanyPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const agentcompanyPlugin = pkgJson["agentcompanyPlugin"];
  if (
    agentcompanyPlugin !== null &&
    typeof agentcompanyPlugin === "object" &&
    !Array.isArray(agentcompanyPlugin)
  ) {
    const manifestRelPath = (agentcompanyPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: AgentCompanyPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumAgentCompanyVersion;
}
 ...
 *   packageName: "agentcompany-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: AgentCompanyPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as AgentCompanyPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a AgentCompany plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<AgentCompanyPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a AgentCompany plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasAgentCompanyPlugin = "agentcompanyPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasAgentCompanyPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<AgentCompanyPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: AgentCompanyPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @agentcompany/shared exports). Run those workers through

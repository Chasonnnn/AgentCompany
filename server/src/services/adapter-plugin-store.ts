 ...
 * ~/.agentcompany/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "@acme/agentcompany-adapter") */
  packageName: string;
...
const AGENTCOMPANY_DIR = path.join(os.homedir(), ".agentcompany");
const ADAPTER_PLUGINS_DIR = path.join(AGENTCOMPANY_DIR, "adapter-plugins");
const ADAPTER_PLUGINS_STORE_PATH = path.join(AGENTCOMPANY_DIR, "adapter-plugins.json");
const ADAPTER_SETTINGS_PATH = path.join(AGENTCOMPANY_DIR, "adapter-settings.json");
...
function ensureDirs(): void {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "agentcompany-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for AgentCompany external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
}

...
import type { AdapterRuntimeServiceReport } from "@agentcompany/adapter-utils";
import type { Db } from "@agentcompany/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@agentcompany/db";
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith("AGENTCOMPANY_")) {
      delete env[key];
    }
  }
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AGENTCOMPANY_WORKSPACE_CWD = input.worktreePath;
  env.AGENTCOMPANY_WORKSPACE_PATH = input.worktreePath;
  env.AGENTCOMPANY_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AGENTCOMPANY_WORKSPACE_BRANCH = input.branchName;
  env.AGENTCOMPANY_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AGENTCOMPANY_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AGENTCOMPANY_WORKSPACE_SOURCE = input.base.source;
  env.AGENTCOMPANY_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AGENTCOMPANY_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AGENTCOMPANY_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AGENTCOMPANY_PROJECT_ID = input.base.projectId ?? "";
  env.AGENTCOMPANY_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AGENTCOMPANY_AGENT_ID = input.agent.id ?? "";
  env.AGENTCOMPANY_AGENT_NAME = input.agent.name;
  env.AGENTCOMPANY_COMPANY_ID = input.agent.companyId;
  env.AGENTCOMPANY_ISSUE_ID = input.issue?.id ?? "";
  env.AGENTCOMPANY_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AGENTCOMPANY_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AGENTCOMPANY_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AGENTCOMPANY_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AGENTCOMPANY_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AGENTCOMPANY_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AGENTCOMPANY_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AGENTCOMPANY_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AGENTCOMPANY_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AGENTCOMPANY_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AGENTCOMPANY_PROJECT_ID = input.workspace.projectId ?? "";
  env.AGENTCOMPANY_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AGENTCOMPANY_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".agentcompany", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "AgentCompany", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,

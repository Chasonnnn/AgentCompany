...
export function productivityReviewService(db: Db, deps?: { enqueueWakeup?: EnqueueWakeup }) {
  ...
  async function resolveReviewOwnerAgentId(sourceIssue: IssueRow, sourceAgent: AgentRow) {
    ...
    const productivityCandidates = await db
      .select({ id: agents.id })
      .from(agents)
      .where(
        and(
          eq(agents.companyId, sourceIssue.companyId),
          inArray(agents.archetypeKey, ["productivity_monitor", "chief_of_staff"]),
          notInArray(agents.status, ["terminated", "pending_approval"]),
        ),
      )
      .orderBy(asc(agents.createdAt), asc(agents.id));
    candidateIds.push(...productivityCandidates.map((agent) => agent.id));

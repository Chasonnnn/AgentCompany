...
import type { BillingType, ExecutionWorkspace, ExecutionWorkspaceConfig } from "@agentcompany/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
import { trackAgentFirstHeartbeat } from "@agentcompany/shared/telemetry";
...
const AGENTCOMPANY_WAKE_PAYLOAD_KEY = "agentcompanyWake";
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[AGENTCOMPANY_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[AGENTCOMPANY_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildAgentCompanyWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  issueSummary?:
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const issueRef = issueContext
      ...
      : null;
    const agentcompanyWakePayload = await buildAgentCompanyWakePayload({
      db,
      companyId: agent.companyId,
    ...
    if (agentcompanyWakePayload) {
      context[AGENTCOMPANY_WAKE_PAYLOAD_KEY] = agentcompanyWakePayload;
    } else {
      delete context[AGENTCOMPANY_WAKE_PAYLOAD_KEY];
    }
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      agentcompanyRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.agentcompanyWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.agentcompanyRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.agentcompanyRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.agentcompanySessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.agentcompanySessionRotationReason = sessionCompaction.reason;
      context.agentcompanyPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.agentcompanySessionHandoffMarkdown;
      delete context.agentcompanySessionRotationReason;
      delete context.agentcompanyPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.agentcompanyRuntimeServices = runtimeServices;
        context.agentcompanyRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[agentcompany] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected AGENTCOMPANY_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.agentcompanyRuntimeServices = combinedRuntimeServices;
        context.agentcompanyRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[agentcompany] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[agentcompany] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
        await releaseIssueExecutionAndPromote(finalizedRun);

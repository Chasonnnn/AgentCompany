...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@agentcompany/db";
 ...
 *
 * This implementation delegates to the core AgentCompany domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@agentcompany/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {

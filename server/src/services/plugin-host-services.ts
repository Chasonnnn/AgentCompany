...
export function buildHostServices(
  ...
  const issueInteractions = issueThreadInteractionService(db);
    ...
    issues: {
      ...
      async getSubtree(params) {
        ...
        const assigneeAgentIds = [
          ...new Set(
            outputIssues.map((issue) => issue.assigneeAgentId).filter((id): id is string => Boolean(id)),
          ),
        ];
        ...
        const [relationPairs, documentPairs, activeRunRows, assigneeRows] = await Promise.all([
          params.includeRelations
            ? Promise.all(
                issueIds.map(async (issueId) => [issueId, await issues.getRelationSummaries(issueId)] as const),
              )
          ...
          params.includeDocuments
            ? Promise.all(
                issueIds.map(async (issueId) => {
                  ...
                  return [issueId, summaries] as const;
                }),
              )
            : Promise.resolve(null),
          ...
          params.includeAssignees && assigneeAgentIds.length > 0
            ? db
                ...
                .from(agentsTable)
                .where(
                  and(eq(agentsTable.companyId, companyId), inArray(agentsTable.id, assigneeAgentIds)),
                )
        ...
        for (const issueId of [...new Set(params.issueIds)]) {
          ...
          const run = await heartbeat.wakeup(issue.assigneeAgentId, {
            ...
            idempotencyKey: params.idempotencyKeyPrefix
              ? `${params.idempotencyKeyPrefix}:${issue.id}`
              : null,
            requestedByActorType: "system",
            requestedByActorId: pluginId,
            contextSnapshot: {
      ...
      async createInteraction(params) {
        ...
        await ensurePluginAvailableForCompany(companyId);
        const issue = requireInCompany("Issue", await issues.getById(params.issueId), companyId);
        const interaction = await issueInteractions.create(issue, params.interaction, {
          agentId: params.authorAgentId ?? null,
          userId: null,
        });
        await logPluginActivity({
          companyId,
          action: "issue.interaction.created",
          entityType: "issue",
          entityId: issue.id,
          actor: { actorAgentId: params.authorAgentId ?? null },
          details: {

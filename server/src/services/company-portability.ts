...
import type { Db } from "@agentcompany/db";
import type {
...
import {
  readAgentCompanySkillSyncPreference,
  writeAgentCompanySkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".agentcompany.yaml" || normalized === ".agentcompany.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const agentcompanyMeta = isPlainRecord(metadata?.agentcompany)
    ? metadata.agentcompany as Record<string, unknown>
    : null;
  return normalizeSkillKey(
    ...
    ?? asString(metadata?.agentcompanySkillKey)
    ?? asString(agentcompanyMeta?.skillKey)
    ?? asString(agentcompanyMeta?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "agentcompany_bundled") {
    return `agentcompany/agentcompany/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "agentcompany" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; AgentCompany will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .agentcompany.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  agentcompanyExtensionPath: string,
) {
  ...
  const extensionEntry = filtered[agentcompanyExtensionPath];
  if (selectedFiles.has(agentcompanyExtensionPath) && typeof extensionEntry === "string") {
    filtered[agentcompanyExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findAgentCompanyExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".agentcompany.yaml"] === "string") return ".agentcompany.yaml";
  if (typeof files[".agentcompany.yml"] === "string") return ".agentcompany.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.agentcompany.yaml") || entry.endsWith("/.agentcompany.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "agentcompany_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "agentcompany/agentcompany",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/chason/agentcompany/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "agentcompany_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.agentcompanySkillKey = skill.key;
  metadata.agentcompany = {
    ...(isPlainRecord(metadata.agentcompany) ? metadata.agentcompany : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const agentcompanyExtensionPath = findAgentCompanyExtensionPath(normalizedFiles);
  const agentcompanyExtension = agentcompanyExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, agentcompanyExtensionPath) ?? "")
    : {};
  const agentcompanyCompany = isPlainRecord(agentcompanyExtension.company) ? agentcompanyExtension.company : {};
  const agentcompanySidebar = normalizePortableSidebarOrder(agentcompanyExtension.sidebar);
  const agentcompanyAgents = isPlainRecord(agentcompanyExtension.agents) ? agentcompanyExtension.agents : {};
  const agentcompanyProjects = isPlainRecord(agentcompanyExtension.projects) ? agentcompanyExtension.projects : {};
  const agentcompanyTasks = isPlainRecord(agentcompanyExtension.tasks) ? agentcompanyExtension.tasks : {};
  const agentcompanyRoutines = isPlainRecord(agentcompanyExtension.routines) ? agentcompanyExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(agentcompanyCompany.brandColor),
      logoPath: asString(agentcompanyCompany.logoPath) ?? asString(agentcompanyCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof agentcompanyCompany.requireBoardApprovalForNewAgents === "boolean"
          ? agentcompanyCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof agentcompanyCompany.feedbackDataSharingEnabled === "boolean"
          ? agentcompanyCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof agentcompanyCompany.feedbackDataSharingConsentAt === "string"
          ? agentcompanyCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(agentcompanyCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(agentcompanyCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: agentcompanySidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(agentcompanyAgents[slug]) ? agentcompanyAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(agentcompanyProjects[slug]) ? agentcompanyProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(agentcompanyTasks[slug]) ? agentcompanyTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(agentcompanyRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(agentcompanyRoutines[slug]) ? agentcompanyRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".agentcompany.yaml" ||
          relative === ".agentcompany.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const agentcompanyAgentsOut: Record<string, Record<string, unknown>> = {};
    const agentcompanyProjectsOut: Record<string, Record<string, unknown>> = {};
    const agentcompanyTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const agentcompanyRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readAgentCompanySkillSyncPreference(
        ...
        agentcompanyAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      agentcompanyProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      agentcompanyTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      agentcompanyRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const agentcompanyExtensionPath = ".agentcompany.yaml";
    const agentcompanyAgents = Object.fromEntries(
      Object.entries(agentcompanyAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const agentcompanyProjects = Object.fromEntries(
      Object.entries(agentcompanyProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const agentcompanyTasks = Object.fromEntries(
      Object.entries(agentcompanyTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const agentcompanyRoutines = Object.fromEntries(
      Object.entries(agentcompanyRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[agentcompanyExtensionPath] = buildYamlFile(
      {
        schema: "agentcompany/v1",
        ...
        agents: Object.keys(agentcompanyAgents).length > 0 ? agentcompanyAgents : undefined,
        projects: Object.keys(agentcompanyProjects).length > 0 ? agentcompanyProjects : undefined,
        tasks: Object.keys(agentcompanyTasks).length > 0 ? agentcompanyTasks : undefined,
        routines: Object.keys(agentcompanyRoutines).length > 0 ? agentcompanyRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, agentcompanyExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeAgentCompanySkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );

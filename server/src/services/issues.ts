  ...
  return {
    ...
    create: async (
      ...
      return db.transaction(async (tx) => {
        ...
        if (issueData.assigneeAgentId && isolatedWorkspacesEnabled) {
          ...
          if (!issueHasEnvironmentSelection) {
          ...
          if (!issueHasEnvironmentSelection && !projectHasEnvironmentSelection) {
            const assigneeAgent = await tx
              ...
              .from(agents)
              .where(and(eq(agents.id, issueData.assigneeAgentId), eq(agents.companyId, companyId)))
              .then((rows) => rows[0] ?? null);

 ...
 */
import { pluginManifestV1Schema } from "@agentcompany/shared";
import type { AgentCompanyPluginManifestV1 } from "@agentcompany/shared";
import { PLUGIN_API_VERSION } from "@agentcompany/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): AgentCompanyPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as AgentCompanyPluginManifestV1,
        };
      }

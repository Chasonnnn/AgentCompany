...
import { readAgentCompanySkillSyncPreference } from "@agentcompany/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@agentcompany/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@agentcompany/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@agentcompany/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveAgentCompanyInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "agentcompany-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "agentcompany-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "agentcompany-feedback-v1";
const FEEDBACK_DESTINATION = "agentcompany_labs_feedback_v1";
...
async function buildCodexTraceFiles(input: {
  ...
  const managedRoot = path.join(
    resolveAgentCompanyInstanceRoot(),
    "companies",
    input.companyId,
    "codex-home",
    "sessions",
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.AGENTCOMPANY_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readAgentCompanySkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let agentcompanyRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      agentcompanyRun = sanitizeFeedbackValue(
        {
          id: run.id,
        ...
        state,
        "bundle.agentcompanyRun",
        MAX_TRACE_FILE_CHARS,
      ) as Record<string, unknown>;

      files.push(makeBundleFile({
        path: "agentcompany/run.json",
        contentType: "application/json",
        source: "paperclip_run",
        contents: `${JSON.stringify(agentcompanyRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.agentcompanyRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "agentcompany/run-events.json",
        contentType: "application/json",
      ...
      if (logText) {
        files.push(makeBundleFile({
          path: "agentcompany/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "paperclip_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.agentcompanyRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {

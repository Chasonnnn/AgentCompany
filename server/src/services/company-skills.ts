...
import { companySkills } from "@agentcompany/db";
import { readAgentCompanySkillSyncPreference, writeAgentCompanySkillSyncPreference } from "@agentcompany/adapter-utils/server-utils";
import type { AgentCompanySkillEntry } from "@agentcompany/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@agentcompany/shared";
import { findActiveServerAdapter } from "../adapters/index.js";
import { resolveAgentCompanyInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("agentcompany/agentcompany/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveAgentCompanyInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "agentcompany_bundled") {
    return {
      editable: false,
      editableReason: "Bundled AgentCompany skills are read-only.",
      sourceLabel: "AgentCompany bundled",
      sourceBadge: "agentcompany",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "AgentCompany workspace",
        sourceBadge: "agentcompany",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "agentcompany_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled AgentCompany skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "agentcompany_bundled"
        && incomingKind === "github"
        && incomingOwner === "agentcompany"
        && incomingRepo === "agentcompany"
      ) {
        out.push(existing);
        continue;
      }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeAgentCompanySkillSyncPreference(config, filtered),
        });
      }
    }

#!/usr/bin/env bash
#
# Kill all local AgentCompany dev server processes (across all worktrees).
#
# Usage:
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.agentcompany/instances/*@agentcompany/db/postmaster.pid
  "$REPO_ROOT"/.agentcompany/instances/*@agentcompany/db/postmaster.pid
  "$REPO_ROOT"/.agentcompany/runtime-services/instances/*@agentcompany/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/paperclip*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.agentcompany/instances/*@agentcompany/db/postmaster.pid
    "$sibling_root"/.agentcompany/runtime-services/instances/*@agentcompany/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No AgentCompany dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} AgentCompany dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to AgentCompany node processes..."

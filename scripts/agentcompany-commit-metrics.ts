...
const DEFAULT_QUERY = "\"Co-Authored-By: AgentCompany <noreply@paperclip.ing>\"";
...
const AGENTCOMPANY_EMAIL = "noreply@paperclip.ing";
const AGENTCOMPANY_NAME = "paperclip";
...
function printHelp() {
  console.log(`Usage: tsx scripts/agentcompany-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === AGENTCOMPANY_EMAIL) || (displayName && displayName.toLowerCase() === AGENTCOMPANY_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("AgentCompany commit metrics");

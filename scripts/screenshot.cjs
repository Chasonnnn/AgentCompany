#!/usr/bin/env node
/**
 * Screenshot utility for AgentCompany UI.
 *
 * Reads the board token from ~/.agentcompany/auth.json and injects it as a
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".agentcompany/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.agentcompany/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the AgentCompany origin only

...
base_cwd="${AGENTCOMPANY_WORKSPACE_BASE_CWD:?AGENTCOMPANY_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${AGENTCOMPANY_WORKSPACE_CWD:?AGENTCOMPANY_WORKSPACE_CWD is required}"
paperclip_home="${AGENTCOMPANY_HOME:-$HOME/.agentcompany}"
paperclip_instance_id="${AGENTCOMPANY_INSTANCE_ID:-default}"
paperclip_dir="$worktree_cwd/.agentcompany"
worktree_config_path="$paperclip_dir/config.json"
worktree_env_path="$paperclip_dir/.env"
worktree_name="${AGENTCOMPANY_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${AGENTCOMPANY_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.agentcompany/config.json" || -L "$base_cwd/.agentcompany/config.json" ) ]]; then
  source_config_path="$base_cwd/.agentcompany/config.json"
fi
...
run_paperclipai_command() {
  local command_args=("$@")
  if command -v pnpm >/dev/null 2>&1 && pnpm agentcompany --help >/dev/null 2>&1; then
    pnpm agentcompany "${command_args[@]}"
    return 0
  fi
  ...
  if command -v agentcompany >/dev/null 2>&1; then
    agentcompany "${command_args[@]}"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  AGENTCOMPANY_DIR="$paperclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  AGENTCOMPANY_WORKTREES_DIR="${AGENTCOMPANY_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const agentcompanyDir = process.env.AGENTCOMPANY_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.AGENTCOMPANY_WORKTREES_DIR) ?? "~/.agentcompany-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(agentcompanyDir, "config.json");
  const envPath = path.resolve(agentcompanyDir, ".env");
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.AGENTCOMPANY_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.AGENTCOMPANY_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.AGENTCOMPANY_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "AGENTCOMPANY_HOME=" + JSON.stringify(worktreeHome),
    "AGENTCOMPANY_INSTANCE_ID=" + JSON.stringify(instanceId),
    "AGENTCOMPANY_CONFIG=" + JSON.stringify(configPath),
    "AGENTCOMPANY_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "AGENTCOMPANY_IN_WORKTREE=true",
    "AGENTCOMPANY_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.AGENTCOMPANY_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("AGENTCOMPANY_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "agentcompany CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi

disable_seeded_routines() {
  local company_id="${AGENTCOMPANY_COMPANY_ID:-}"
  if [[ -z "$company_id" ]]; then
    echo "AGENTCOMPANY_COMPANY_ID not set; skipping routine disable post-step." >&2
    return 0
  fi

  if ! run_paperclipai_command routines disable-all --config "$worktree_config_path" --company-id "$company_id"; then
    echo "agentcompany CLI not available in this workspace; skipping routine disable post-step." >&2
  fi
}
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".agentcompany-backup-$BASHPID"
    moved_symlink_paths=()

...
const devServerStatusFilePath = path.join(repoRoot, ".agentcompany", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".agentcompany/dev-server-status.json",
]);
...
const env = {
  ...process.env,
  AGENTCOMPANY_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.AGENTCOMPANY_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.AGENTCOMPANY_MIGRATION_PROMPT ??= "never";
  env.AGENTCOMPANY_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.AGENTCOMPANY_DEPLOYMENT_MODE = "authenticated";
  env.AGENTCOMPANY_DEPLOYMENT_EXPOSURE = "private";
  env.AGENTCOMPANY_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@agentcompany/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclip] Command failed with code ${status.code}: pnpm --filter @agentcompany/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.AGENTCOMPANY_MIGRATION_AUTO_APPLY === "true";
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@agentcompany/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@agentcompany/server", serverScript, ...forwardedArgs],

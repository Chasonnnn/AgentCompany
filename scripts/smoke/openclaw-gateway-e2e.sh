...
AGENTCOMPANY_API_URL="${AGENTCOMPANY_API_URL:-http://127.0.0.1:3100}"
API_BASE="${AGENTCOMPANY_API_URL%/}/api"
...
AGENTCOMPANY_API_URL_FOR_OPENCLAW="${AGENTCOMPANY_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${AGENTCOMPANY_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${AGENTCOMPANY_AUTH_HEADER}" )
fi
if [[ -n "${AGENTCOMPANY_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${AGENTCOMPANY_COOKIE}" )
  AGENTCOMPANY_BROWSER_ORIGIN="${AGENTCOMPANY_BROWSER_ORIGIN:-${AGENTCOMPANY_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${AGENTCOMPANY_BROWSER_ORIGIN}" -H "Referer: ${AGENTCOMPANY_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set AGENTCOMPANY_COOKIE or AGENTCOMPANY_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg agentcompanyApiUrl "$AGENTCOMPANY_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        agentcompanyApiUrl: $agentcompanyApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    ...
    assert_status "200"
    {
      echo "AGENTCOMPANY_API_URL=${AGENTCOMPANY_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set AGENTCOMPANY_API_KEY=${AGENT_API_KEY} in your run context before running AgentCompany heartbeat steps."
...
main() {
  ...
  wait_http_ready "${AGENTCOMPANY_API_URL%/}/api/health" 15 || fail "AgentCompany API health endpoint not reachable"

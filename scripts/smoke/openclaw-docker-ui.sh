...
AGENTCOMPANY_HOST_PORT="${AGENTCOMPANY_HOST_PORT:-3100}"
AGENTCOMPANY_HOST_FROM_CONTAINER="${AGENTCOMPANY_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.agentcompany-openclaw.override.yml"
...
detect_paperclip_base_url() {
  ...
  for candidate in "$AGENTCOMPANY_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${AGENTCOMPANY_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${AGENTCOMPANY_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
AgentCompany URL for OpenClaw container:
EOF
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${AGENTCOMPANY_HOST_PORT}
  (Do not use http://127.0.0.1:${AGENTCOMPANY_HOST_PORT} inside the container.)
  If AgentCompany rejects the host, run on host machine:
    pnpm agentcompany allowed-hostname host.docker.internal
  Then restart AgentCompany and re-run this script.
EOF
  fi
  cat <<EOF

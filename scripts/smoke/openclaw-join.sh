...
COMPANY_ID="${COMPANY_ID:-${AGENTCOMPANY_COMPANY_ID:-}}"
...
Provide one of:
  AGENTCOMPANY_AUTH_HEADER=\"Bearer <board-token>\"
  AGENTCOMPANY_COOKIE=\"<board-session-cookie>\"
...
log "checking AgentCompany health"
...
assert_status "200"
if ! grep -q "AgentCompany OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.agentcompany.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"

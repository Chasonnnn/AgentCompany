...
AGENTCOMPANY_DEPLOYMENT_MODE="${AGENTCOMPANY_DEPLOYMENT_MODE:-authenticated}"
AGENTCOMPANY_DEPLOYMENT_EXPOSURE="${AGENTCOMPANY_DEPLOYMENT_EXPOSURE:-private}"
AGENTCOMPANY_PUBLIC_URL="${AGENTCOMPANY_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e AGENTCOMPANY_DEPLOYMENT_MODE="$AGENTCOMPANY_DEPLOYMENT_MODE" \
      -e AGENTCOMPANY_DEPLOYMENT_EXPOSURE="$AGENTCOMPANY_DEPLOYMENT_EXPOSURE" \
      -e AGENTCOMPANY_PUBLIC_URL="$AGENTCOMPANY_PUBLIC_URL" \
      -e AGENTCOMPANY_HOME="/paperclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "agentcompany@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$AGENTCOMPANY_HOME" --base-url "$AGENTCOMPANY_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $AGENTCOMPANY_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$AGENTCOMPANY_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$AGENTCOMPANY_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$AGENTCOMPANY_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$AGENTCOMPANY_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$AGENTCOMPANY_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$AGENTCOMPANY_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $AGENTCOMPANY_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $AGENTCOMPANY_DEPLOYMENT_MODE/$AGENTCOMPANY_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$AGENTCOMPANY_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $AGENTCOMPANY_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$AGENTCOMPANY_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $AGENTCOMPANY_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"

...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("agentcompany-worktree-enabled") !== "true") return null;

  const name = readMetaContent("agentcompany-worktree-name");
  const color = normalizeHexColor(readMetaContent("agentcompany-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("agentcompany-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}

...
import { parseAgentMentionHref, parseProjectMentionHref, parseSkillMentionHref } from "@agentcompany/shared";
...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--agentcompany-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--agentcompany-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("agentcompany-mention-chip", `agentcompany-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("agentcompany-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "agentcompany-mention-chip",
    "agentcompany-mention-chip--agent",
    "agentcompany-mention-chip--project",
    "agentcompany-mention-chip--skill",
    "agentcompany-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--agentcompany-mention-project-color");
  element.style.removeProperty("--agentcompany-mention-icon-mask");
}

...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace>): ExecutionWorkspace {
  return {
    ...
    cleanupState: overrides.cleanupState ?? "idle",
    cleanupAttemptCount: overrides.cleanupAttemptCount ?? 0,
    lastCleanupError: overrides.lastCleanupError ?? null,
    nextCleanupAttemptAt: overrides.nextCleanupAttemptAt ?? null,
    reconcileState: overrides.reconcileState ?? "pending",

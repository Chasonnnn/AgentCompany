...
/**
 * Filter .agentcompany.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterAgentCompanyYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://github.com/chason/agentcompany) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm agentcompany company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [AgentCompany](https://github.com/chason/agentcompany) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [AgentCompany](https://github.com/chason/agentcompany) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .agentcompany.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .agentcompany.yaml
    const yamlPath = exportData.agentcompanyExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterAgentCompanyYaml(exportData.files[yamlPath], checkedFiles);
    }

...
describe("CompanyAccess", () => {
  ...
  it("keeps the page human-focused and explains implicit versus explicit grants", async () => {
    ...
    const optionLabels = Array.from(document.body.querySelectorAll("option")).map((option) => option.textContent?.trim());
  ...
  it("removes a member with an issue reassignment target", async () => {
    ...
    const reassignmentSelect = selects.find((select) =>
      Array.from(select.querySelectorAll("option")).some(
        (option) => option.getAttribute("value") === "__unassigned",
      ),
    );
    expect(reassignmentSelect).toBeTruthy();
    await act(async () => {
      reassignmentSelect!.value = "user:user-1";
    ...
    expect(archiveMemberMock).toHaveBeenCalledWith("company-1", "member-2", {
      reassignment: { assigneeAgentId: null, assigneeUserId: "user-1" },
    });

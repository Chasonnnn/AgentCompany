...
function mergeRuns(runs: RunForIssue[], liveRuns: LiveRunForIssue[] | undefined, activeRun: ActiveRunForIssue | null | undefined) {
...
function compactAgentName(run: LedgerRun, agentMap: ReadonlyMap<string, Agent>) {
  return run.agentName ?? agentMap.get(run.agentId)?.name ?? run.agentId.slice(0, 8);
}

function summarizeCost(runs: LedgerRun[]) {
  let input = 0;
  let output = 0;
  let cached = 0;
  let cost = 0;
  ...
  for (const run of runs) {
    ...
    const result = asRecord(run.resultJson);
    const runInput = readNumber(usage?.inputTokens) ?? readNumber(usage?.input_tokens) ?? 0;
    const runOutput = readNumber(usage?.outputTokens) ?? readNumber(usage?.output_tokens) ?? 0;
    const runCached =
...
export function IssueRunLedger({
  ...
  const ledgerRuns = useMemo(() => mergeRuns(runs, liveRuns, activeRun), [runs, liveRuns, activeRun]);
  const latestRun = ledgerRuns[0] ?? null;
  const children = childIssueSummary(childIssues);
  const issueCostSummary = useMemo(() => summarizeCost(ledgerRuns), [ledgerRuns]);
  ...
  return (
    <section className="space-y-3" aria-label="Issue run ledger">
      ...
      {(issueCostSummary.hasCost || issueCostSummary.hasTokens) ? (
        <div className="rounded-md border border-border/70 px-3 py-2 text-xs text-muted-foreground tabular-nums">
          <div className="mb-1 font-medium text-foreground">Cost summary</div>
          <div className="flex flex-wrap gap-3">
            {issueCostSummary.hasCost ? <span>${issueCostSummary.cost.toFixed(4)}</span> : null}
            ...
            return (
              <article key={run.runId} className="space-y-2 px-3 py-3">
                ...
                <div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">
                  <div>
                    <span className="text-foreground">Elapsed</span> {duration ?? "unknown"}
                  </div>
                  <div>
                    <span className="text-foreground">Last useful action</span> {lastUsefulActionLabel(run)}
                  </div>
                  <div>
                    <span className="text-foreground">Stop</span> {stopStatusLabel(run, stopReason)}
                  </div>
                </div>

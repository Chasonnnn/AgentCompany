...
type AgentCompanySprite = (typeof AGENTCOMPANY_SPRITES)[number];
...
function spriteSize(sprite: AgentCompanySprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AGENTCOMPANY_SPRITES[(baseCol + baseRow) % AGENTCOMPANY_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AGENTCOMPANY_SPRITES[Math.floor(Math.random() * AGENTCOMPANY_SPRITES.length)]!;

...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@agentcompany/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@agentcompany/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@agentcompany/adapter-utils").TranscriptEntry[];

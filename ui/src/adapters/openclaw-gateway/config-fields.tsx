...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="AgentCompany API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "agentcompanyApiUrl",
                  String(config.agentcompanyApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "agentcompanyApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://agentcompany.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "agentcompany"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="agentcompany"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. AgentCompany persists a device key during onboarding so pairing approvals
